const CandidateResume = {
  recommend_reason: {
    recommend_reason: "为何推荐此职位",
    overall: "Overall",
    competency_match: "Competency",
    career_aspirations_match: "Aspirations",
    summary: "Summary",
    detailed_alignment_analysis: "Detailed Alignment Analysis",
  },
  career_aspirations: {
    metadata: "元数据",
    transcript_date: "访谈记录日期",
    profile_date: "候选人档案日期",
    career_context_and_status: "职业背景与现状",
    current_career_situation_summary: "当前职业状况概述",
    overall_career_trajectory_goal: "整体职业发展轨迹目标",
    job_search_status: "求职状态",
    search_urgency_and_timeline_details: "求职紧迫程度与时间线详情",
    motivators_and_values: "动机与价值观",
    primary_motivators_for_next_role: "下一份工作的主要驱动力",
    key_values_sought_in_work_environment: "期望的工作环境核心价值观",
    target_opportunity_profile: "目标机会画像",
    desired_role_title: "期望的职位名称",
    ideal_role_characteristics: "理想角色特征",
    desired_scope_and_impact_summary: "期望影响力与范围概述",
    desired_seniority_level_summary: "期望职级水平概述",
    skills_to_utilize: "希望使用的技能",
    skills_to_develop: "希望发展的技能",
    strategic_vs_execution_focus_preference: "战略 vs 执行侧重偏好",
    ideal_company_characteristics: "理想公司特征",
    excluded_industries: "排除行业",
    preferred_company_size_stage_summary: "期望的公司规模/阶段概述",
    desired_company_culture_summary: "期望的公司文化概述",
    public_private_company_preference: "上市公司/私营公司偏好",
    desired_overall_contribution_summary: "期望的整体贡献概述",
    boundaries_and_firm_preferences: "边界与明确偏好",
    non_negotiables_deal_breakers: "不可妥协项（底线）",
    strong_preferences_flexible: "强烈但可协商的偏好",
    practical_considerations: "实际考量",
    minimum_base_salary_expectation: "最低基本薪资预期",
    target_base_salary_expectation: "目标基本薪资预期",
    target_total_compensation_expectation: "目标总薪酬预期",
    other_compensation_components_sought: "其他期望的薪酬组成部分",
    compensation_context_or_flexibility: "薪酬背景或灵活性",
    notice_period_details: "通知期详情",
    availability_to_start_details: "入职可用性详情",
    location_and_mobility: "地点与流动性",
    preferred_locations: "期望工作地点",
    willingness_to_relocate_details: "是否愿意搬迁及详情",
    work_mode_preference_validated: "验证后的工作模式偏好",
    work_mode_details: "工作模式详情",
    preferred_work_environment_language: "期望的工作语言环境",
    preferred_industries: "期望行业",
    target_function_or_work_type: "目标职能或工作类型",
  },
  candidate_resume: {
    profile: "简历",
    pending: "简历生成中...",
    basic_info: "基本信息",
    email: "邮箱：",
    phone: "电话：",
    base_in: "目前所在地：",
    work_authorization: "工作许可：",
    hightlight: "候选人亮点",
    career_summary: "职业总结",
    core_career_track: "核心职业轨迹",
    narrative: "描述",
    primary_work_environments: "主要工作环境",
    primary_work_languages: "主要工作语言",
    core_competencies: "核心能力",
    other_competencies: "其他能力",
    work_experiences: "工作经验",
    employer_context: "雇主背景:",
    core_role_objectives: "岗位核心目标/职责：",
    team_context: "团队背景：",
    projects_involved: "参与的项目：",
    key_achivements: "关键成就：",
    summary: "总结：",
    reason_for_leaving_or_current_status: "离职原因：",
    education: "教育经历",
    additional_qualifications: "其他资质",
    publications: "发表作品",
    certifications: "证书",
    awards_and_honors: "获得奖项/荣誉",
    resume: "简历",
    biography: "个人传记",
    discovery_chat_done: "对话已完成，简历正在生成中，请稍候...",
  },
  candidate_aspirations: {
    career_aspirations: "职业目标",
    pending: "职业目标生成中...",
    desired_role: "期望职位",
    minimum_salary: "最低薪资",
    maximum_salary: "最高薪资",
    remote_preference: "远程工作偏好",
    preferred_locations: "期望工作地点",
    preferred_industries: "期望行业",
    job_search_status: "求职状态",
  },
  candidate_home: {
    home: "首页",
    recommended_jobs: "为您推荐的职位",
    important_tasks: "重要任务",
    deep_aspirations: "深度职业规划",
    deep_aspirations_hint: "帮助我了解您的职业规划，以便为您推荐最合适的职位。",
    work_experience_hint:
      "帮助我深入了解您的工作经历，以便为您推荐最合适的职位。",
  },
  job_applies: {
    jobs: "适合您的职位",
    applied_jobs: "您的职位",
  },
  candidate_jobs: {
    tabs: {
      recommend: "为你推荐的职位",
      apply: "你已申请的职位",
    },
    empty_description: "Viona 正在努力为你寻找合适的职位。",
    status: {
      interview_scheduled: "已确认面试",
      interview_created: "待确认面试",
      accepted: "简历已通过",
      rejected: "简历未通过",
      screening: "简历筛选中",
      ai_dialogue: "AI 对话",
    },
    role_type: {
      onsite: "完全在办公室工作",
      hybrid: "混合型",
      remote: "完全在家工作",
    },
  },
  job_apply: {
    title: "您的职位",
    jd: "职位描述",
    progress: "职位申请进度",
    applied: "已申请",
    apply_now: "立即申请",
    job_description: "职位描述",
    interview: "面试",
    finish_interview_hint: "面试已完成，您可以提交申请了！",
    waiting: "Viona 正在撰写中，请稍候...",
    chat: "聊聊公司/职位",
    posted_at: "发布于 {{time}}",
    respond_interview: "确认第一轮面试安排",
    company_overview: "公司简介",
    position_overview: "职位概览",
    basic_requirements: "基本要求",
    bonus_points: "加分项",
    progress_title: "职位申请进度",
    chat_cta_label: "开始沟通",
    whatsapp_confirmed_hint:
      "感谢您！我们的 AI 招聘官 <b>Viona</b> 已通过 WhatsApp 与您联系（消息会显示为 <b>Viona by Persevio</b>）。<br /><br />请在方便时回复她，完成初步沟通，以便我们在提交给雇主前准确完善您的申请资料。",
    human_mode_hint: "<b>感谢您的配合。</b><br />顾问很快会致电与您沟通。",
    whatsapp_confirm_title: "准备好与 Viona 聊天了吗？",
    whatsapp_confirm_hint: `Viona 已准备好通过 WhatsApp 与您联系，助您快速推进申请。`,
    confirm_interview_title: "确认面试时间",
    confirm_interview_success: "面试时间确认成功。",
    first_round_interview: "第一轮面试",
    interview_time: "面试时间",
    interview_modal: {
      name: "面试名称",
      mode: "面试类型",
      duration: "面试时长",
      minutes: "分钟",
      interviewer: "面试官",
      time: "面试时间",
    },
    steps: {
      apply: {
        title: "提交职位申请",
      },
      chat: {
        title_human: "与人工顾问预约沟通",
        title_ai: "与 AI 招聘官即时沟通",
        hint_human:
          "人工顾问会尽快与您电话联系，进一步了解您的背景和求职意向。",
        hint_ai:
          "在提交简历前，您将与 Viona 进行一次简短互动，确认岗位是否匹配您的背景与兴趣，确保投递更合适的机会。",
      },
      screening: {
        title: "简历筛选",
        hint: "招聘团队和用人部门正在筛选简历。",
      },
      processed: {
        accepted: "简历已通过",
        rejected: "简历未通过",
        default: "简历已处理",
      },
    },
    interview_arrangement: {
      confirm_success: "面试时间确认成功",
      confirm_failed: "面试时间确认失败",
      interview_starting_on: "{{count}} 个面试开始于",
      morning: "上午",
      afternoon: "下午",
    },
  },
  candidate_sign: {
    connect_google: "使用 Google 登录",
    connect_linkedin: "使用 LinkedIn 登录",
    connect_title:
      "结束无尽的求职之旅。Persevio 使用 AI 帮您匹配优质机会，尤其在",
    singapore: "新加坡",
    connect_intro_title_1: "一次对话，多个机会：",
    connect_intro_content_1:
      "与我们的 AI 招聘顾问 Viona 聊一次天，我们将主动向您发送高度精准、个性化的职位推荐。",
    connect_intro_title_2: "隐私有保障",
    connect_intro_content_2:
      "您的资料保持私密。只有当您决定申请某个具体职位时，雇主才能看到您的信息。",
    connect_intro_title_3: "全程引导式申请流程",
    connect_intro_content_3:
      "Viona 将作为您的专属 AI 助手，在每一步为您提供支持。",

    confirm_contact: "确认联系方式",
    confirm_contact_hint:
      "请确保您的联系信息正确无误。招聘流程后续步骤中我们需要联系您。",
    next: "下一步",

    email_exists: "邮箱已存在",
    email_not_exists: "邮箱不存在",
    upload_resume: "上传简历",
    career_dive: "深入职业探索",

    upload_your_resume: "上传您的简历",
    upload_succeed: "上传成功",
    upload_failed: "上传失败",
    upload_tips_1: "请上传一份您的简历副本或下载的 LinkedIn 简历。",
    upload_tips_2:
      "即使简历不是最新的也无需担心！这份文档不会发送给雇主。它只是为我们接下来的对话提供一个起点。聊天结束后，我们会为您特定的求职需求起草一份全新的、更完善的简历。",
  },

  apply_job: {
    title: "与我们先聊一聊，准备好您的申请",
    title_without_job: "让我们一起构建您的职业档案",
    hint: "在我们提交您的申请之前，我们的顾问【账号名-Viona-Ai Recruiter】将通过 Whatsapp 与您进行一次简短的沟通，以便更深入地了解您的经历、技能和职业偏好，帮助我们以最有利的方式向雇主介绍您。",
    hint_without_job: `为了推荐最适合您的职位，我们的 AI 招聘官 <b>Viona</b> 将通过 WhatsApp（来自 WhatsApp 账户 "Viona by Persevio"）与您进行一次简短的发现对话。这将帮助我们验证您的经验、澄清简历中的关键细节并了解您的职业偏好，以便我们能够准确有效地向雇主介绍您。`,
    list_confidentiality:
      "本次沟通仅用于为您提供更好的服务，不会涉及敏感隐私信息，绝不会泄露给第三方或用于营销目的。",
    list_add_contact:
      "为了在 WhatsApp 上为您提供专属服务，请将招聘顾问添加到您的 WhatsApp 联系人。",
    whatsapp_label: "您的 Whatsapp 账号",
    placeholder: "请输入您的账号",
    required_message: "请输入您的 Whatsapp 账号",
    pattern_message: "请输入有效的 Whatsapp 账号（仅数字）",
    previous_step: "上一步",
    next: "在 WhatsApp 上聊天",
    agree_whatsapp_contact: "我同意就我的求职申请通过 WhatsApp 与我联系",
    agree_platform_terms_lead: "我同意 ",
    terms_of_service: "服务条款",
    agree_platform_terms_conj: " 与 ",
    privacy_policy: "隐私政策",
    modal_terms_of_service: "服务条款",
    modal_privacy_policy: "隐私政策",
    agreement_required_toast: "请先阅读并同意相关协议",
    skip_no_whatsapp: "我没有 WhatsApp",
  },
};

export default {
  hello: "世界",
  new: "创建",
  edit: "编辑",
  save: "保存",
  delete: "删除",
  retry: "重试",
  cancel: "取消",
  submit: "提交",
  submitting: "提交中...",
  submit_succeed: "提交成功",
  submit_failed: "提交失败",
  confirm: "确认",
  close: "关闭",
  add: "添加",
  create_succeed: "创建成功",
  create: "创建",
  update: "更新",
  update_failed: "更新失败",
  update_succeed: "编辑成功",
  delete_success: "删除成功",
  delete_failed: "删除失败",
  save_success: "保存成功",
  save_failed: "保存失败",
  back: "返回",
  chat_with_viona: "和 Viona 聊聊",
  created_at: "创建时间",
  chat_title: "对话",

  accept: "通过",
  reject: "拒绝",
  accepted: "已通过",
  rejected: "已拒绝",

  copy: "复制",
  copied: "复制成功",

  loading: "加载中...",
  uploading: "上传中...",

  year: "年",
  empty_text: "暂无数据",

  download: "下载",

  numberRange: {
    min: "最小值",
    max: "最大值",
  },

  date_format: {
    with_day: "ddd, MM月DD日",
  },

  pagination_total: "第 {{rangeStart}}-{{rangeEnd}} 条，共 {{total}} 条",

  please_select: "请选择",
  please_enter: "请输入",
  phone_not_valid: "请输入有效的手机号",

  calendar: {
    all_day: "全天",
    add_times: "添加时间",
    add_time_title: "添加时间",
    months: {
      jan: "一月",
      feb: "二月",
      mar: "三月",
      apr: "四月",
      may: "五月",
      jun: "六月",
      jul: "七月",
      aug: "八月",
      sep: "九月",
      oct: "十月",
      nov: "十一月",
      dec: "十二月",
    },
    weekdays: {
      sun: "日",
      mon: "一",
      tue: "二",
      wed: "三",
      thu: "四",
      fri: "五",
      sat: "六",
    },
  },

  interview_mode_options: {
    interview: "面试",
    written: "笔试",
  },

  job_apply_mode_options: {
    ai: "AI",
    human: "Human",
    whatsapp: "Whatsapp",
  },

  switch_mode_reason_options: {
    dissatisfied_with_my_answer: "对我的回答不满意",
    want_to_consult_more_complex_questions: "想咨询一些更复杂的问题",
    want_to_chat_with_a_real_person: "想和真人聊天以获得更多确认",
    others: "其他",
  },

  assessment_options: {
    meets: "满足",
    partially_meets: "部分满足",
    uncertain: "不确定",
    does_not_meet: "不满足",
  },

  pipeline_section: {
    reached_out: "已接触",
    applied: "已申请",
    started_ai_interview: "已开始 AI 面试",
    ai_interview_completed: "AI 面试已完成",
    shortlisted: "已入围",
    rejected: "已淘汰",
  },

  sourcing_channel: {
    persevio: "Persevio",
    direct: "Direct",
    linkedin: "LinkedIn",
    jobstreet: "JobStreet",
    mycareersfuture: "MyCareersFuture",
    email_inbound: "邮件申请",
    jobstreet_email: "Jobstreet (邮箱)",
    indeed_email: "Indeed (邮箱)",
    linkedin_email: "LinkedIn (邮箱)",
    fast_jobs_email: "Fast Jobs (邮箱)",
    my_careers_future_email: "MyCareersFuture (邮箱)",
    glassdoor_email: "Glassdoor (邮箱)",
  },
  // ------------------------------ 以下是非通用文案 ------------------------------
  menu: {
    referrals: "推荐",
    newRole: "发布职位",
    dashboard: "Dashboard",
    jobs: "职位",
    company: "公司基本信息",
    career_page: "招聘主页",
    interviewer: "面试官",
    talents: "候选人",
    settings: "设置",
    account_management: "账号管理",
    group_management: "分组管理",
    org_chart: "组织架构",
    member_team: "成员与团队",
    job_management: "职位管理",
    talent_management: "候选人管理",
    job_apply_management: "申请管理",
    candidate_list: "C端用户列表",
    candidate_connection_list: "匹配列表",
    company_list: "公司列表",
    daily_breakdown: "每日统计",
    tools: "工具",
    kb_observation: "KB Observation 列表",
    kb_entity_document: "KB Entity Document 列表",
    mcf_jobs: "MCF 职位",
    credit_config: "Credit Config",
    credit_package: "Credit Package",
    credits: "Credits",

    home: "首页",
    profile: "简历",
    aspirations: "职业规划",
    deep_aspirations: "深度职业规划",
    job_applies: "职位申请",
    connections: "匹配/会面",
    switch_to_admin_mode: "切换到猎头端",
    switch_to_employer_mode: "切换到雇主端",
  },

  job: {
    chat: "对话",
    document: "职位详情 & 资料",
    talents: "候选人",
  },

  share_job: {
    loading: "加载中...",
    notFound: "未找到该职位",
    listSeparator: "、",
    basicInfo: {
      title: "基本信息",
      location: "工作地点",
      roleType: "办公类型",
      level: "级别",
    },
    sections: {
      requirement: "详细职位信息",
      description: "职位描述",
      email: "邮件内容",
    },
    level: {
      internship: "实习生",
      no_experience: "应届毕业生/无经验",
      junior: "初级/少量经验",
      mid_level: "中级/有一定经验",
      senior: "高级/经验非常丰富",
    },
  },

  chat: {
    tips: "小贴士：你可以直接把你的回答写在我的问题后面/下面，或者直接在这里编辑我建议的草稿。",
    yes: "好的。",
    no: "没有/不需要。",
    accurate: "准确。",
    proposal: "你的提议不错。",
    no_others: "没什么其它的了。",
    confirm: "确认",
    agree: "没有修改，提议很好。",

    // 新增的国际化键值
    error_message: "Viona 出现了问题，请重试。",
    quota_exhausted: "您的配额已耗尽。",
    get_job_failed: "获取工作信息失败。",
    send_role_overview_failed: "发送角色概览失败。",
    delete_message_success: "删除消息成功。",
    delete_message_failed: "删除消息失败。",
    upload_failed: "上传失败。",
    confirm_delete_messages: "确认删除此消息之后的所有消息？",
    confirm_retry_message: "确认重试此消息？",
    interview_feedback_confirm_text:
      "如果您已经审阅好当前的面试评分卡，请确认归档；也可以告诉我对评分、理由或其他分析内容的调整。",
    recording_tooltip:
      "长按【Ctrl】键可直接与Viona对话（备注：连按两次 Ctrl 键即可快速启动录音，再单次按下则结束录音）",
    you: "您",
    ok: "确定",

    // CandidateChat 组件相关文案
    finish_conversation: "完成对话",
    message_delete_success: "消息删除成功",
    message_delete_failed: "消息删除失败",
    voice_input: "语音输入",
    stop_voice_input: "停止语音输入",
    voice_input_hint_title: "语音输入",
    voice_input_hint_content: "您可以通过以下方式快速启用语音输入功能:",
    voice_input_hint_method1: "长按【Ctrl】键进行持续语音输入",
    voice_input_hint_method2:
      "连按两次【Ctrl】键快速启动录音，再次按下结束录音",
    confirm_delete_message:
      "确定删除这条消息吗？删除后，该条消息之后的聊天记录不可恢复。",
    voice_chat: "语音对话",
    text_edit: "文字编辑",
    click_again_to_stop_recording: "再次点击停止录音",
    release_ctrl_to_stop_speaking: "松开",
    ctrl_key: "Ctrl",
    to_stop_speaking: "停止说话",
    press_and_hold: "按住",
    to_speak: "说话",
    release_to_stop_speaking: "松开停止说话",
    press_and_hold_to_speak: "按住说话",
    jrd_step_understand_role: "理解职位",
    jrd_step_align_icp: "对齐理想候选人画像",
    jrd_step_pitch_logistics: "职位介绍",
    jrd_step_left: "剩余",

    task: "任务",
    next_task: "下一个任务: ",
    jd_next_task:
      "有了详细的职位需求文档，您批准的面试计划和职位描述（JD），我现在可以准确地向潜在的候选人介绍我们公司和这个「职位名称」职位，并且回答他们关于这个职位的任何问题了。你可以直接把下面的链接附在JD上发到招聘网站上，也可以直接发给潜在的候选人。剩下的就交给我吧！",
    required_task_next_text: "您的下一个任务是：",
    jrd_next_task: "下面是您还没完成的任务，请告诉我您下一步想进行哪项任务：",
    create_job: "创建职位",
    define_job_requirement: "详细定义职位需求",

    define_target_companies: "确定目标企业",
    define_compensation_details: "确定薪资",
    define_screening_questions: "简历筛选问题",
    define_interview_plan: "确定面试计划",
    define_outreach_message: "确定邮件内容",
    define_social_post: "确定社交素材",
    define_faq: "如何吸引候选人",
    draft_job_description: "写职位描述（JD）",
    create_chatbot: "候选人Chatbot",
    evaluate_result: "简历评估",
    recommend_interview_question: "推荐面试问题",
    generate_interview_feedback: "生成面试评价",

    viona_intro_candidate: "your application copilot",
    viona_intro_staff: "AI招聘专员",

    share_basic: "点击补充基本信息",
    share_reference:
      "您可以点击按钮上传文件/拖放文件，或者直接把 JD 文字粘贴在下方的文本输入框中发送",
    share_team: "点击分享团队信息",
    ideal_profile: "点击编辑理想候选人画像",
    other_requirements: "点击确定其它要求",
    salary_structure: "点击确定薪资结构",
    screening_questions: "点击编辑筛选问题",
    view_jrd: "查看详细职位需求文档",
    view_jd: "查看职位描述文档",
    view_compensation_details: "查看薪资结构文档",
    view_outreach_message: "查看邮件内容文档",
    view_interview_plan: "查看面试计划文档",
    view_talent: "查看候选人",
    no_edits: "没有要修改的",
    copy_link: "邀请同事加入对话",
    invite_collaborators_cta: "邀请协作者",
    invite_btn: "邀请",
    reply_group: "发送群消息…",
    cannot_mention_viona_while_thinking:
      "Viona 仍在思考中，请去掉 @Viona 或稍后再试。",
    mention_empty: "没有可 @ 的对象",
    mention_title: "提及",
    mention_role_you: "You",
    mention_role_owner: "Owner",
    mention_role_ai: "AI",
    mention_role_hiring_manager: "Hiring Manager",
    mention_role_recruiter: "Recruiter",
    mention_role_guest: "Guest",
    invite_collaborators: {
      title: "邀请协作者",
      hint: "把链接分享给协作者。对方打开后需要填写名字，并等待你批准后才能加入对话。",
      copy_link: "复制链接",
      copied_btn: "已复制",
      copied: "链接已复制",
      refresh_link: "刷新链接",
      refreshed: "链接已刷新",
      refresh_failed: "刷新链接失败",
      done: "完成",
    },
    intake_members: {
      in_conversation: "对话中共 {{count}} 人",
      role_owner: "Owner",
      role_ai: "AI",
      role_staff: "Hiring Manager",
      role_hiring_manager: "Hiring Manager",
      role_recruiter: "Recruiter",
      role_guest: "Guest",
      remove: "移除",
      remove_title: "移除成员",
      remove_content: "确定将「{{name}}」移出对话吗？",
      removed: "已移除",
      remove_failed: "移除失败",
      role_updated: "角色已更新",
      role_update_failed: "更新角色失败",
    },
    open: "打开聊天机器人",
    jd_done: "创建给候选人介绍职位的聊天机器人",
    draft_job_description_btn: "写职位描述（JD）",

    percy_is_thinking: "Percy 正在努力思考！请稍等，她马上就会回复...",
    percy_is_thinking_first_message:
      "Percy 正在仔细审查您的简历，以便根据您的经验量身定制对话。这可能需要 1-5 分钟...",
    viona_is_thinking: "Viona正在努力思考！请稍等，她马上就会回复...",
    viona_is_thinking_first_message:
      "Viona 正在仔细审查您的简历，以便根据您的经验量身定制对话。这可能需要 1-5 分钟...",
    viona_is_generating_jrd_strategy:
      "<b>Viona 正在分析提供的职位细节，以便提出最相关的问题。这通常需要 30–90 秒</b>",
    jd_progress_label: "职位 intake 已完成，现在开始<strong>起草 JD</strong>..",
    jd_progress_aria: "正在起草职位描述",
    intake_mode_ask: `开始之前，有两种 intake 模式可选。您想用哪一种？

**1. 标准模式**（强烈推荐 · 约 20 分钟）：
我们会详细梳理职位：职责、要求、如何向候选人介绍这个岗位。**深度信息能让我更精准地寻源和筛选**，所以我强烈推荐。

**2. 快速模式**（约 10 分钟）：
快速 briefing。我会基于您提供的材料，问几个关键问题并确认要求。适合需求清晰的岗位，或您已经详细梳理过的职位。`,
    intake_mode_standard_button: "标准模式（约 20 分钟）",
    intake_mode_fast_button: "快速模式（约 10 分钟）",
    edit_profiles_hint:
      "我已经编辑了理想候选人画像，通过添加、删除或修改内容修订了你的提议。",
    reply_viona_directly_or_edit: "回复 Viona 的消息",
    reply_viona: "回复 Viona",
    reply_percy: "回复 Percy",

    edit_message: "编辑消息",
    edit_message_desc: "点击这里修改 Viona 的消息，或者直接在问题下面添加答案",
    edit_ideal_profile: "编辑理想候选人画像",
    edit_real_requirement: "编辑真实需求",
    edit_target_candidate_profile: "编辑目标候选人画像",
    view_strategy: "查看策略",
    reset_state: "重置状态",
    confirm_reset_state: "确定要重置状态吗？",
    reset_success: "状态重置成功。请删除上传 JD 的消息并重新上传。",
    reset_failed: "重置失败",
    job_requirement_strategy_title: "职位需求对话策略",

    chatbot_greeting:
      "我现在可以准确地向潜在的候选人介绍我们公司和这个「职位名称」职位，并且回答他们关于这个职位的任何问题了。你可以直接把下面的链接附在JD上发到招聘网站上，也可以直接发给潜在的候选人。剩下的就交给我吧！",
    edit_screening_question_hint:
      "我已经编辑了简历筛选问题，通过添加、删除或修改内容修订了你的提议。",

    chatbot_greeting_1: `Viona 能够直接与候选人沟通。她可以介绍我们的公司，回答关于职位的问题，并通过直接聊天帮助将好奇的潜在候选人转变为感兴趣的申请人。

您可以通过此聊天页面链接分享 Viona，可以将其包含在您的职位发布中，或直接发送给候选人：`,
    chatbot_greeting_2:
      "为确保 Viona 在与候选人互动前拥有全面的信息，我们强烈建议您完成以下设置任务：",
    chatbot_greeting_3:
      "您也可以在此处配置信息披露指南，具体规定 Viona 可以与候选人分享哪些信息以及信息的详细程度。",
    required: "必填",
    optional: "选填",

    question_predefined_1: "跟我相对简略地介绍下这个职位的核心职责和上下文。",
    question_predefined_2: "简单介绍下你们公司。",
    question_predefined_3: "这个职位要达成的核心目标是什么？",
    question_predefined_4: "介绍下这个职位要加入的团队。",
    question_predefined_5: "你们想要招聘的理想候选人是什么样的？",

    question_context: "跟我相对简略地介绍下这个职位的核心职责和上下文。",
    question_company: "简单介绍下你们公司。",
    question_objectives: "这个职位要达成的核心目标是什么？",
    question_team: "介绍下这个职位要加入的团队。",
    question_candidate: "你们想要招聘的理想候选人是什么样的？",
    question_interview: "面试流程是怎么样的？",
    question_location: "这个职位的工作地点是在哪？",

    copied: "已复制",
    edit_message_hint:
      "我在下方通过添加/删除/修改你的原始回复，已经回答了你的问题、修改了你的提案或提供了我的反馈。改动可能很小，请务必仔细审阅，确保没有遗漏任何内容。",

    require_compensation: "请先完成薪资配置",
    require_interview_plan: "请先完成面试计划",
    chatbot_config: "配置聊天机器人",

    gather_basic_information: "获取职位基本信息",
    confirm_key_role_parameters: "确定关键职位参数",
    define_ideal_candidate_profile: "定义理想画像",
    define_screening_criteria: "最终对齐",

    essential: "必要",
    optional_task: "非必要",
    candidate: "候选人",

    post_job_greeting_1:
      "将您的职位发布在 Persevio 上，我们将开始为您匹配高度合适的、经过预先筛选的候选人，助力您招聘到理想人选。",
    post_job_greeting_2: `Persevio 与传统招聘网站有何不同？

a. 获取专属人才：接触到顶级候选人，包括那些并未在 LinkedIn 或其他招聘平台上积极求职的被动型专业人士。我们的独特模式能够吸引到你在其他地方找不到的高素质人才。

b. 节省时间，候选人已预先筛选：我们推荐的每一位候选人都经过全面的初步面试筛选，评估其经验和适合度，确保您只会见到合格的人选。

c. 只接收相关职位申请：您的职位发布不会公开。我们仅将职位分享给经过筛选、技能和经验高度匹配的一组候选人，避免大量不合适的申请涌入。

d. 获得更深入的候选人洞察：超越简历本身。对于每位推荐的候选人，您都将收到一份详细的分析报告。

e. 免费发布职位：您可以免费发布职位空缺，开始发现卓越人才，无需任何费用。`,
    post_job_btn: "发布您的职位",
    post_job_done: "您的职位已发布到 Persevio",
    post_job: "发布职位",

    extract_high_level_responsibility: "编辑核心职责清单",
    extract_day_to_day_tasks: "编辑日常具体工作清单",
    extract_icp: "确定候选人评估标准",
    extract_success_metric: "编辑成功指标",

    high_level_responsibility: "核心职责清单",
    day_to_day_tasks: "日常具体工作清单",
    view_document: "查看文档",
    realities: "职位现实",
    responsibilities: "职位职责",
    icp: "候选人画像",
    highlights: "亮点",
    "role-essence": "角色精髓",
    cddreq: "候选人要求",
    "cdd-profile": "理想候选人画像",
    sourcing: " sourcing 指南",
    summary: "职位需求",
    jd: "职位描述",

    recording: "正在录音",
    transcribing: "正在转录",
  },

  job_requirement_form: {
    tips: "<b style='color: black'>下面的这些问题不是必填</b>。但是您提供给我的信息越多，我就能更好地了解这个职位。",
    basic_information: "基本信息",
    org_node_question: "该职位所属的组织/部门是？",
    org_node_placeholder: "请选择组织节点（可选）",
    org_node_save_failed: "保存职位所属部门失败，请重试",

    // basic_info 题目文案
    headcount_number_question: "这个职位有多少个 headcount？",
    employment_arrangement_question: "这个职位是正式还是合同制？",
    employment_arrangement_options: {
      permanent: "正式（Permanent）",
      contract: "合同制（Contract）",
      contract_to_perm: "合同转正（Contract-to-perm）",
    },
    contract_duration_question: "合同期限是多久？",
    work_time_question: "这个职位是全职还是兼职？",
    work_time_options: {
      full_time: "全职（Full-time）",
      part_time: "兼职（Part-time）",
    },
    hours_required_question: "每天/每周需要工作多少小时？",
    role_budget_question: "这个职位的预算是多少？",
    visa_requirement_question: "这个职位的签证要求是什么？",
    visa_must: "Must",
    visa_preferred: "Preferred",
    visa_must_placeholder: "Must",
    visa_preferred_placeholder: "Preferred",
    visa_at_least_one_message: "请至少填写 Must 或 Preferred 其中一项",
    hard_starting_date_question: "这个职位是否有硬性最晚到岗日期要求？",
    hard_starting_date_placeholder: "候选人需要在……之前到岗",
    role_open_duration_question: "这个职位已经开放多久了？",
    role_open_duration_options: {
      just_opened: "刚开放",
      under_a_month: "不到一个月",
      one_to_three_months: "1–3 个月",
      over_three_months: "超过 3 个月",
    },
    primary_driver_question: "开设这个职位的主要驱动力是什么？",
    primary_driver_multi_question:
      "开设这个职位的主要驱动力是什么？可多选。",
    speed_quality_tradeoff_question:
      "当速度与质量出现冲突时，这个职位更看重哪一边？",
    archetypes_question:
      "以下三种角色类型中，哪种最能描述这个职位的主要关注点？",
    language_question: "这个职位将主要使用哪些语言？",
    visa_question: "这个职位支持哪些工作签证类型？",
    team_question: "这个职位将加入哪个团队？",
    manager_question: "这个职位的直属领导是谁?",
    employee_type_question: "这个职位的级别是什么？",
    experience_years_question: "这个职位需要多少年的工作经验？",
    percentage_question: "这个职位的工作性质和贡献类型是什么？",
    influence_question: "这个职位将产生什么级别的影响？",
    remote_question:
      "这个职位是<b>完全在办公室工作</b>，还是<b>完全在家工作</b>，还是<b>混合型</b>？",
    role_type_question: "这是一个新职位还是填补空缺？",
    key_people_question: "这个职位需要与哪些关键人员合作才能取得成果？",
    city_question: "这个职位的<b>办公地点</b>是?",

    // 选项文案
    primary_driver_options: {
      backfill: "**填补空缺**：替换已离职的员工以维持当前产出。",
      team_growth:
        "**团队增长/扩展**：为现有成功团队增加容量以处理更多工作量。",
      new_initiative:
        "**新项目/计划**：为新团队、部门或业务领域引入第一个成员。",
      capability_gap:
        "**能力缺口**：为新战略产品线、业务单元或实验部门引入具有创业精神的高技能个人。",
      specialized_expertise:
        "**特殊专业技能**：引进具有高度专业知识的高技能个人，以推动创新和专业知识。",
      leadership: "**领导力/继任**：我们需要为现有团队或职能引入新领导。",
    },

    primary_driver_multi_options: {
      backfill: "**填补空缺**：替换已离职或即将离职的人员",
      team_expansion:
        "**团队扩张**：因工作量增长，为现有团队增加人力",
      first_hire_new_function:
        "**新职能的首个招聘**：目前无人从事该工作——此人将开启这一职能",
      new_initiative_project:
        "**新举措/新项目**：由特定新产品、新市场、新业务线或新中标合同驱动",
      missing_expertise:
        "**引入缺失能力**：团队需要一项当前不具备的特定技能或领域专长",
      leadership_hire:
        "**领导力招聘**：此人将带领团队或负责某职能——也包括引入未来接班人",
      performance_upgrade:
        "**绩效升级**：我们希望这个席位的表现显著高于以往水准。",
      restructuring:
        "**组织调整**：该职位因重组而产生或被重塑——职责被合并、拆分或迁移",
      contract_or_regulation:
        "**合同或合规要求**：客户合同、牌照或合规义务要求设立该席位",
      other: "**其他**：其他原因",
    },

    speed_quality_tradeoff_options: {
      speed_first:
        "**速度优先** — 空缺席位正在造成损失；尽快招到合适的人，好过追求完美但更晚到岗。",
      bar_first:
        "**标准优先** — 这个招聘太重要，不能降低标准；宁可多等，也要等到卓越人选。",
      balanced:
        "**二者兼顾** — 在降低标准之前我们愿意延长搜寻，但不会无限期拖延。",
    },

    archetypes_options: {
      individual_contributor:
        "**个人贡献者**：主要职责是亲自动手创建、构建或交付工作。成功以个人产出和工艺卓越来衡量。",
      people_manager:
        "**人员管理者**：主要职责是管理、指导和培养直接下属团队。成功以团队的整体表现、健康度和产出衡量。",
      ic_and_pm:
        "**个人贡献者 + 人员管理者**: 主要职责是领导团队通过直接执行和团队发展。成功以高质量的个人产出和团队集体交付与增长衡量。",
      business_function_owner:
        "**业务/职能负责人**：主要职责是拥有广泛的业务成果、损益、产品或战略计划。成功以业务影响和实现战略目标衡量。",
    },

    // option texts
    employee_level: {
      internship: "**实习生**",
      no_experience: "**应届毕业生**:无经验",
      junior: "**初级**:少量经验",
      mid_level: "**中级**:有一定经验",
      senior: "**高级**:经验非常丰富",
    },

    role_type_options: {
      new_role: "新职位",
      backfill: "填补空缺",
    },

    key_people_options: {
      working_independently: "独立工作：独立完成工作，不需要太多与他人互动。",
      direct_team: "直接团队：他们的直接同事或直属经理。",
      cross_team:
        "跨团队/跨职能/跨部门同事：公司内其他团队、职能、部门的同事。",
      company_leadership:
        "公司领导层：团队领导、部门主管、总监、副总裁、C级高管。",
      external_customers:
        "外部客户、客户和最终用户：使用或可能使用我们产品或服务的人。",
      external_partners: "外部合作伙伴和供应商：与我们合作的外部方。",
      external_stakeholders: "外部利益相关者：如外部投资者。",
    },

    percentage_options: {
      execution: "执行层面：主要负责执行上级明确指派的、具体且定义清晰的任务。",
      project:
        "项目/目标管理：负责独立或协作推动一个完整的项目从启动到交付，或达成一个具体的业务目标，需要跨职能协调与资源整合。",
      team_management:
        "管人/管团队：对一整个团队的最终工作结果负责。负责团队的日常管理、人才培养、绩效评估、招聘与激励，确保团队协同高效地达成目标。",
      strategy:
        "设计高层战略和工作计划：负责或深度参与公司或某个业务线中长期发展战略、业务计划的讨论、制定与落地。",
    },

    influence_options: {
      task_level:
        "**任务执行层面影响：**这个职位的影响仅限于分配给他们任务，其成功标准是任务的完成质量和效率。",
      project_level:
        "**项目层面影响：**此职位是特定项目成功的关键贡献者或负责人，其工作质量直接决定项目能否按时、按质、按预算交付。",
      team_level:
        "**团队级影响：**此职位是其所在团队的整体业绩和目标的负责人或核心贡献者，其产出将显著影响团队是否能达成其工作目标。",
      cross_functional:
        "**跨团队/部门协同级影响：**此职位需要主导或协调多个团队/整个部门的合作，其工作成效直接关系到跨团队协同的效率和部门整体目标的实现。",
      business_level:
        "**业务线/产品线级别：**此职位的决策和成果将直接塑造某一产品线或业务线的市场表现和商业成功，直接影响该业务的收入、市场份额或用户增长。",
      company_level:
        "**公司级别影响：**此职位的工作将对公司的整体战略、财务状况或市场声誉产生重要影响，其成败关系到公司的长期发展和核心竞争力。",
    },

    reason: "职位创建原因",
    back_fill: "填补空缺",
    new_role: "新职位",
    employment_type: "聘用形式",
    perm: "长期正式员工",
    contract: "短期合同用工",
    duration: "合同时长（月）",
    time: "工作时间要求",
    full_time: "全职（每周至少大于40小时）",
    part_time: "兼职",
    working_hours: "每周要求工作时间",
    on_site: "完全在办公室工作",
    remote: "完全在家工作",
    hybrid: "混合型",
    intership: "实习生/培训生/入门岗位/校招岗位",
    junior: "初级社招",
    senior: "资深社招",
    manager: "团队lead/经理",
    director: "总监/部门负责人",
    executive: "公司高管/领导团队",
    work_experience: "理想情况下，这个职位需要有多少年的工作经验？",
    internal_employee: "这个职位有内部职级吗？",
    head_count: "计划总共招聘多少人员（headcount）？",
    urgency: "职位紧急度/优先度?",

    reference: "参考资料",
    materials:
      "任何你认为可以帮我建立起一个对这个职位初步印象的信息都可以在这分享给我。",
    usage:
      "我该怎么用这个参考资料？比如，告诉我这是个你写的简单的职位需求表？别的类似的职位？里面哪些信息我可以用，哪些不能用？",
    no_materials: "没有可提供的材料",

    team_context: "团队信息",
    team: "这个职位会加入哪个<b>团队</b>？",
    create_team: "创建团队",
    team_details: "团队信息",
    team_name: "团队名称",
    team_intro:
      "介绍下这个团队的主要目标和职责？一个详细的团队介绍可以帮助Viona更好地理解这个职位",
    members_count: "不算这个新招的职位，现在这个团队有多少人？",
    menbers_detail: "简单介绍下团队成员，他们各自的职责，资历，等。",
    team_language: "团队的工作语言。",
    manager_detail: "这个职位的汇报给谁？",

    other_requirements: "其它要求",
    visa: "签证要求",
    country: "国家",
    china: "中国",
    singapore: "新加坡",
    visa_type: "支持的签证类型",
    singapore_citizen: "Singapore Citizen",
    singapore_pr: "Singapore PR",
    ep: "EP",
    sp: "SP",
    wp: "WP",
    dp: "DP",
    other_singapore_visa: "其它",
    chinese_citizen: "中国公民",
    china_pr: "中国永居",
    no_need_visa: "无需重新申请工作签证",
    need_visa: "需要重新申请工作签证",
    other_visa: "其它",
    visa_type_singapore_other: "其它签证类型",

    language_group: "语言要求",
    language: "语言",
    language_placeholder: "选择或输入以添加语言",
    language_add_custom: '添加「{{name}}」',
    chinese: "中文",
    english: "英语",
    proficiency: "流利程度",
    proficiency_level_5: "**Level 5**：母语/完全双语",
    proficiency_level_4: "**Level 4**：高级专业 — 可主导复杂商务讨论",
    proficiency_level_3: "**Level 3**：职能专业 — 可参与会议并处理往来函件",
    proficiency_level_2: "**Level 2**：有限工作 — 仅简单对话",
    proficiency_level_1: "**Level 1**：基础 — 问候和简单短语",
    native_speaker: "母语",
    professional: "专业水准",
    daily_conversation: "可以日常沟通",
    proficiency_other: "其它",
    proficiency_other_name: "其它",

    travel_group: "出差",
    need_travel: "是否需要出差",
    no_travel: "不用出差",
    hoc_travel: "一般不用，零星有具体需要时",
    some_travel: "偶尔出差",
    regular_travel: "经常出差",
    destination: "出差目的地",
    nature: "出差性质",
    regularity: "出差频率",

    onboarding: "onboard日期要求",
    certification: "职业证书，安全要求，合规要求等",
    other: "其它",
    requirement: "要求",

    create_team_succeed: "团队创建成功",
    required_error_message: "请输入或者选择",

    add: "添加{{name}}",

    salary_structure: "薪资结构",
    base_salary: "基础工资",
    salary_number: "金额",
    salary_by_result: "按结果/工作量计算工资",
    bonus: "奖金",
    bonus_frequency: "发放频率",
    yearly: "每年",
    semi_annually: "每半年",
    quarterly: "每季度",
    monthly: "每月",
    bonus_other: "其他",
    bonus_number: "奖金数目",
    bonus_type: "发放条件",
    bonus_note: "注释",
    commission: "佣金",
    equity: "股份",
    allowances: "补贴",
    social_security_contributions: "社保",
    benefits_perks: "福利",
    insurance: "保险",
    paid_time_off: "假期 （年假，病假，产假等）",
    benefits_perks_other: "其它福利",
    salary_other: "其它",
    salary: "月薪",
    months: "月数",
    month: "月",

    no_data: "没有/不需要。",
    year: "年",
    percentage_message: "请填写百分比，且百分比之和必须等于100",
    manager_message: "请填写职位和姓名",
    city_and_address_message: "请至少选择一个城市和地址。",
  },

  select_options: {
    core_requirements: "核心要求",
    plus_points: "加分项",

    high_level_responsibility_alert: "请选择或者调整职责内容，以满足您的预期",
    day_to_day_alert:
      "请选择或者调整当前职责中最重要、最具代表性的任务，以满足您的预期。",
    success_metric_alert: "请选择或者调整成功指标，以满足您的预期。",
    core_responsibility: "核心职责",
    secondary_responsibility: "次要职责",
    primary_metric: "主要指标",
    secondary_metric: "次要指标",
  },

  profile: {
    not_a_match: "不匹配",
    recommend_with_reservations: "推荐",
    match_pending_verification: "总体合适，但需要确认一些信息",
    match: "匹配",
    strong_match: "非常匹配",

    met: "满足",
    not_met: "不满足",
    not_sure: "不确定",

    VH: "非常高",
    H: "高",
    N: "普通",
    L: "低",
    VL: "非常低",

    show_result: "查看评估结果",
    download_resume: "下载简历",
    reason: "判断原因",
    evidence: "相关简历原文",
    overall: "综合",
    competency: "能力",
    logistics: "逻辑",
    reasoning: "理由",
    rank: "评价",
    bonus: "额外加分",
    total: "总计",
    confidence_level: "信心",
  },

  career_page: {
    title: "招聘主页",
    enable: "启用招聘主页",
    switch_on: "招聘主页当前已开启",
    switch_off: "招聘主页当前已关闭",
    domain_name: "招聘主页域名",
    domain_hint: "3–15 个字符：a–z、_ 或 -",
    domain_required: "请填写招聘主页域名",
    domain_length: "域名长度必须为 3-15 个字符",
    domain_chars: "域名只能包含字母 (a-z)、下划线 (_) 和连字符 (-)",
    page_title: "页面标题",
    page_title_required: "请填写页面标题",
    introduction: "公司介绍",
    save: "保存",
    saved: "保存成功",
    save_failed: "保存招聘主页设置失败",
    copied: "已复制",
    copy_link: "复制招聘主页链接",
    preview: "预览招聘主页",
    status_on: "开启",
    status_off: "关闭",
  },

  credits: {
    pageTitle: "Credits",
    availableCredits: "Available Credits",
    equivalentTo: "Equivalent to S${{amount}}",
    unavailableLabel: "Unavailable",
    awaitingActivationTooltip: "{{amount}} awaiting activation",
    expiringLabel: "Expiring",
    viewExpiringCredits: "View expiring credits",
    noCreditsExpiring: "No credits expiring",
    transactionHistory: "Transaction History",
    filterType: "Type",
    filterSource: "Source",
    filterAll: "All",
    typeIncoming: "Incoming",
    typeOutgoing: "Outgoing",
    columnTime: "Time",
    columnType: "Type",
    columnSource: "Source",
    columnAmount: "Amount",
    columnDescription: "Description",
    columnTotalAfter: "Total After",
    noData: "No data",
  },

  company: {
    name: "公司名称",
    knowledge_base: "信息库",
    website: "公司链接",
    language: "公司系统语言",
    hint: {
      title:
        "您的人工智能招聘顾问Viona，需要依靠本数据库中的信息来执行以下关键任务：",
      li1: "在和用人经理沟通过程中理解职位要求。",
      li2: "创建准确且具吸引力的职位描述。",
      li3: "针对候选人关于公司的问询，提供准确的有依据的回答。",
      li4: "以及许多其他任务。",
      footer:
        "请确保所有公司信息都全面、准确并保持最新。不完整或非最新的信息将会影响 Viona 的工作效能。",
    },
    logo: "Logo",
    upload_logo_succeed: "Logo 上传成功",
    upload_logo_failed: "Logo上传失败",
    status: {
      approving_title: "审核中",
      rejected_title: "审核未通过",
      approving_message: "您的注册申请还在审核中,请耐心等待。",
      rejected_message:
        "很抱歉，您的账号未能通过审核；您可以联系 admin@persevio.ai 获取帮助。",
    },
  },

  settings: {
    profile: "设置",
    name: "姓名",
    email: "邮件",
    logout: "登出",
    password: "密码",
    change_password: "更改密码",
    update_password_success: "密码更新成功",
    update_password_error: "密码更新失败",
    update_lang_success: "语言更新成功",
    update_lang_error: "语言更新失败",
    update_timezone_success: "时区更新成功",
    update_timezone_error: "时区更新失败",
    language_and_time: "语言与时间",
    language: "显示语言",
    language_label: "显示语言",
    language_tooltip:
      "自定义显示语言：可随时切换当前个人账号的界面显示语言，以满足个人使用习惯。",
    time_zone: "时区",
    system_time_zone_prefix: "您当前的系统时区是 {{- timezone}}。",
    use_this_time_zone: "使用此时区",
  },

  create_job: {
    new_role: "创建职位",
    job_name: "职位名称",
    viona_title: "AI 招聘专员",
    ready_message: "准备好发布职位了吗？请告诉我职位名称吧！",
    ready_message_prefix: "我已经准备好开始这次寻访了。首先，请问这个岗位的",
    ready_message_emphasis: "职位名称",
    ready_message_suffix: "是什么？",
    try_asking: "试试这些：",
    suggestion_1: "软件工程师",
    suggestion_2: "销售经理",
    composer_placeholder: "请输入职位的官方名称",
    share_message: "您也可以分享链接，与小伙伴一起定义职位需求",
    share_link: "分享链接",
    copy_success: "复制成功",
    reply_placeholder: "请输入职位的官方名称，例如：软件工程师、销售经理等",
    create_success: "创建职位成功",
  },

  group_chat_invite: {
    invalid_title: "邀请链接已失效",
    invalid_description: "请向职位负责人索取新链接，或前往 Persevio 首页。",
    go_home: "前往 Persevio 首页",
    landing_title: "{{name}} 邀请你一起创建职位",
    landing_description:
      "加入对话，与 Persevio AI 招聘官 Viona 一起讨论职位需求。",
    join_as_guest: "以访客身份加入",
    name_placeholder: "你的名字",
    join_conversation: "加入对话",
    or: "或",
    sign_in: "登录 Persevio",
    access_note: "登录后的协作者在对话结束后仍可访问该职位。",
    join_failed: "加入对话失败",
    leave: "离开",
    leave_title: "离开对话",
    leave_content: "确定要离开这场 Intake 对话吗？",
    leave_confirm: "离开",
    leave_failed: "离开对话失败",
    left_title_prefix: "你已离开职位 Intake 对话",
    left_thanks: "感谢你的参与。",
    left_ask_owner: "请向 {{name}} 索取新链接以重新加入",
    left_ask_owner_fallback: "请向职位负责人索取新链接以重新加入",
    intake_pill: "Job Intake",
    this_job: "该职位",
  },

  job_board: {
    publish_to_persevio: "发布",
    complete_jd_first: "请先完成职位描述(JD)任务",
    operation_success: "操作成功",
    link_copied: "链接已复制",
    unfinished_tasks: "您还有{{count}}项任务要完成",
    detailed_define_job_requirement: "详细定义职位需求",
    define_jd: "确定职位描述(JD)",
    create_interview_plan: "制定面试计划&评分卡",
    recommend_candidate_questions: "推荐候选人面试问题",
    fill_candidate_scorecard: "填写候选人评分卡",
    complete_conversation_tasks:
      "与 Viona 完成对话任务，以下是为您生成的详细文档：",
    job_requirement_table: "职位需求表",
    job_description_jd: "职位描述(JD)",
    job_compensation_details: "薪资结构",
    job_outreach_message: "邮件内容",
    define_compensation_details: "确定薪资",
    define_outreach_message: "确定邮件内容",
    interview_plan_scorecard: "面试计划&评分卡",
    no_candidates_yet:
      "您现在还没有候选人，可以告诉我候选人信息，我们一起制定面试计划或者填写评分卡！",
    upload_resume: "上传简历",
    parse_resume_failed: "解析简历失败",
    upload_failed: "上传失败",
    candidate_exists_confirm: "已存在候选人{{name}}，请确认是否继续上传",
    create_succeed: "创建成功",
    submit_failed: "提交失败",
    candidates_list:
      "以下是您的候选人，可以制定面试计划或者填写评分卡！也可以与Viona对话新增新的候选人。",
    delete_confirm: "请确认是否删除 {{name}} ，删除后原有内容无法恢复?",
    delete_success: "删除成功",
    delete_failed: "删除失败",
    publish_to_persevio_recruitment: "发布到Persevio招聘网站",
    recruitment_chatbot: "招聘chatbot",
    copy_job_requirement_link:
      "复制【职位需求】链接，用于HR、用人经理、面试官等定义职位需求、生成面试反馈",
    post_job: "确认发布",
    step_create_new_job: "创建新职位",
    step_role_basics: "角色基础信息",
    step_role_briefing: "角色背景资料",
    step_job_intake: "职位需求",
    step_draft_jd: "起草JD",
    step_preview: "预览",
    step_publish: "发布",
    preview_description:
      '职位预览已生成。请仔细审阅信息。确认无误后，点击页面底部的"确认发布"按钮，将职位发布到 Persevio 招聘网站。',
    published_successfully: "发布成功！",
    published_success_content:
      "您的职位现已上线并接受申请！您可以在职位详情页查看和编辑职位信息。",
    go_to_job_details_page: "前往职位详情页",
    role_basics_title: "角色基础信息",
    role_basics_hint: "请先填写职位基础信息，提交后将保存为 basic_info 文档。",
    role_briefing_title: "角色背景资料",
    role_briefing_hint:
      "您可以上传文件、手动输入或语音输入。系统会将两部分内容拼接后保存为 reference。",
    role_briefing_upload_title: "上传文件",
    role_briefing_upload_hint:
      "点击或拖拽文件到此处提取文本（支持 PDF、DOCX、TXT、MD）",
    role_briefing_manual_title: "手动输入",
    role_briefing_text_placeholder: "请输入职位背景说明，或使用语音输入...",
    role_briefing_submit: "保存并继续",
    role_briefing_parse_failed: "文件解析失败",
    role_briefing_file_uploaded: "文件解析成功",
    role_briefing_file_parsing: "正在解析 {{fileName}}...",
    role_briefing_file_uploaded_name: "已上传文件：{{fileName}}",
    role_briefing_empty_content: "请先上传文件或输入内容",
    role_briefing_file_type_error:
      "不支持的文件类型，请上传 PDF、DOCX、TXT 或 MD",
  },

  job_chat: {
    detailed_define_job_requirement: "详细定义职位需求",
    define_jd: "定义 JD",
    define_compensation_details: "确定薪资",
    define_outreach_message: "确定邮件内容",
    define_interview_plan: "定义面试计划&评分卡",
  },

  job_talents: {
    creator: "创建者",
    status_init: "初始状态",
    status_extracting: "提取中",
    status_extracted: "已提取",
    candidate_name: "候选人姓名",
    evaluate_result: "匹配度",
    screening_status: "筛选状态",
    schedule_time: "面试时间",
    feedback: "拒绝原因",
    interview_mode: "面试模式",
    interview: "面试",
    written: "笔试",
    status_message_sent: "已发送消息",
    status_message_read: "已读消息",
    status_pending: "待处理",
    status_staff_accepted: "待安排面试",
    status_staff_rejected: "未通过",
    status_interview_scheduled: "已安排面试",
    status_interview_confirmed: "已确认面试",
    action: "操作",
    view: "查看",
    candidate_list: "候选人列表",
    company_name: "公司名称",
    job_name: "职位名称",
    years_of_experience: "工作经验",
    current_compensation: "当前薪资",
    expected_compensation: "期望薪资",
    visa: "签证",
    received_on: "申请时间",
    evaluate_result_options: {
      absolutely: "理想候选人",
      yes: "匹配",
      yes_but: "可能",
      maybe: "可能",
      no: "不匹配",
    },
    skills_fit_options: {
      ideal: "优秀",
      good: "良好",
      overqualified: "超额匹配",
      near_fit: "接近匹配",
      uncertain: "不确定",
      poor: "不匹配",
    },
    logistics_fit_options: {
      no_issues: "无问题",
      location_mismatch: "地点不匹配",
      work_arrangement_mismatch: "工作安排不匹配",
      travel_constraint: "旅行限制",
      needs_sponsorship: "需要赞助",
      above_budget: "超出预算",
      long_notice: "长期通知",
      availability: "可用性",
      other_constraint: "其他限制",
    },
    work_experiences: "工作经历",
    search_placeholder: "请输入候选人姓名进行搜索",
    job_placeholder: "职位",
    level_placeholder: "匹配度",
    assigned_recruiters: "指定 Recruiter",
    assigned_recruiters_placeholder: "点击指定...",
    recruiter_select_empty_hint: "请先在职位设置的协作者中添加招聘专员",
  },

  job_document: {
    job_details: "职位详情",
    updated_at: "更新",
    link_copied: "链接已复制",
    copied: "已复制",
    chat_with_viona: "与 Viona 对话",
    cancel: "取消",
    save: "保存",
    submit_succeed: "提交成功",
    submit_failed: "提交失败",
    not_written_job_requirement: "暂未撰写职位需求表",
    not_written_job_description: "暂未撰写职位描述(JD)",
    not_created_interview_plan: "暂未制定面试计划&评分卡",
    job_requirement_table: "职位需求表",
    job_description_jd: "职位描述(JD)",
    job_compensation_details: "薪资结构",
    job_outreach_message: "邮件内容",
    interview_plan_scorecard: "面试计划&评分卡",
  },

  jobs_show: {
    expand: "展开",
    job_at: "职位在",
    click_for_complete_jd: "点击查看完整职位描述",
    days_ago: "天前",
    today: "今天",
    job_description: "职位描述",
    powered_by_persevio: "由 Persevio 提供技术支持",
    role_type: {
      onsite: "完全在办公室工作",
      hybrid: "混合型",
      remote: "完全在家工作",
    },
    apply_now: "感兴趣",
    hours_ago: "小时前发布",
    days_ago_posted: "天前发布",
    team_language: "团队语言",
    job_description_section: "职位描述",
    basic_requirements: "基本要求",
    bonus_points: "加分项",
    ask_me_anything: "有任何问题都可以问我",
    job_closed_description_line1:
      "该职位目前已不再接受新的申请，您可以查看其他感兴趣的职位。",
    job_closed_description_line2: "一旦有新的匹配职位，我们将第一时间通知您。",
    complete_registration_first: "请先完成注册流程。",
    apply_job_failed: "申请职位失败",
    viona_intro_text:
      "你好，我是 Viona，负责该职位的 AI 招聘官，对公司或岗位有疑问尽管问我！",
    drawer_title: "Viona，您的求职副驾",
    updated_at: "更新时间：{{date}}",
    share_token_copied: "专属链接已复制",
    back_to_career_page: "返回招聘主页",
  },

  coworker: {
    description: `请和您的AI招聘专员Viona对话，以详细定义「{{jobName}}」这个职位的职位要求和上下文。`,
  },

  candidate_screening_question: { edit_screening_questions: "完成问卷" },

  job_information: {
    basic: "职位基本信息",
    jrd: "完整职位需求文档",
    jd: "职位描述",
    target_companies: "目标公司",
    compensation_details: "薪资结构",
    screening_question: "简历筛选问题",
    interview_plan: "面试计划",
    outreach_message: "候选人触达信息",
    social_media: "社交网络帖子",
    faq: "如何吸引候选人",
    updated_at: "更新于 ",
  },

  talent: {
    not_a_match: "不匹配",
    recommend_with_reservations: "推荐",
    match_pending_verification: "总体合适，但需要确认一些信息",
    needs_verification: "需要确认一些信息",
    match: "匹配",
    strong_match: "非常匹配",

    met: "满足",
    not_met: "不满足",
    not_sure: "不确定",

    VH: "非常高",
    H: "高",
    N: "普通",
    L: "低",
    VL: "非常低",

    resume_body: "简历正文",
    evaluate_report: "简历评估报告",
    summary: "评估总结",
    overall: "总体评估结果",
    compensation: "能力适合度",
    other: "其它指标适合度",
    rank: "候选人排名",
    details: "详细分析",
    details_requirement: "具体职位要求分析",
    confidence: "信心",
    reason: "判断原因",
    evidence: "简历相关原文",
    accept: "通过",
    reject: "拒绝",
    feedback: "反馈",
    accepted: "已通过",
    rejected: "已拒绝",

    // 新增的talent页面相关翻译
    arrange_interview: "安排面试",
    interview_name: "面试名称",
    interview_type: "面试类型",
    interview_duration: "面试时长",
    interviewer: "面试官",
    interview_time: "面试时间",
    waiting_candidate_choice: "等待候选人选择",
    scheduled_interviews: "已安排面试",
    minutes: "分钟",
    add_interview: "添加面试",
    name: "名称",
    type: "类型",
    duration_minutes: "时长(分钟)",
    interviewer_field: "面试官",
    available_times: "可选时间",
    online_interview: "线上面试",
    offline_interview: "线下面试",
    candidate_details: "候选人详情",
    edit_context: "编辑上下文",
    interview_plan_scorecard: "请先指定面试计划&评分卡",
    recommended_interview_questions: "推荐面试问题",
    fill_scorecard: "填写评分卡",
    round: "轮次",
    upload_resume_or_select_candidate:
      "您可以上传简历或选择一位候选人，与我一起制定该候选人的面试计划。",

    // 状态相关
    status_evaluate_succeed: "评估成功",
    status_accepted: "已通过",
    status_rejected: "已拒绝",

    // 日期格式
    date_format: "YYYY-MM-DD HH:mm",

    // 其他通用文案
    loading: "加载中...",
    submit: "提交",
    cancel: "取消",

    // SelectOrUploadTalent 组件相关
    select_candidate: "选择候选人",
    or: "或",
    upload_resume: "上传简历",
    upload_failed: "上传失败",
    upload_succeed: "上传成功",
    candidate_exists_confirm: "已存在候选人{{name}}，请确认是否继续上传",

    // TalentDetail 组件相关
    talent_status: {
      strong_hire: "强烈推荐：超出标准。高度自信。",
      hire: "推荐：符合标准。有信心。",
      hold: "保留：优秀候选人，但可能在时间、级别或特定职位需求上存在不匹配。未来职位可再次考虑。",
      no_hire_underqualified:
        "不予录用（资历不足）：未达到核心技能或经验水平要求。",
      no_hire_not_a_fit:
        "不予录用（不匹配）：具备所需技能，但特质、工作方式或动机与职位/公司不匹配。",
    },
    signal_level: {
      must_have: "必须具备",
      good_to_have: "加分项",
    },
    tabs: {
      resume_detail: "简历详情",
      recommended_interview_questions: "推荐面试问题",
      interview_scorecard: "面试评分卡",
    },
    candidate_evaluation_report: "候选人评估报告",
    recommended_interview_questions_title: "推荐面试问题",
    round_label: "Round {{round}}",
    update_time: "更新时间：",
    download_filename: "Round {{round}} - 推荐面试问题.md",
    copy_success: "已复制到剪贴板",
    copy_failed: "复制失败",
    link_copied: "链接已复制",
    chat_with_viona: "与 Viona 对话",
    no_interview_plan: "暂未获取推荐面试计划",
    interview_scorecard_title: "面试评分卡",
    final_decision_and_reason: "最终决定与理由",
    overall_recruitment_committee_recommendation: "总体招聘委员会推荐",
    final_reason: "最终理由",
    update_success: "更新成功",
    interview_feedback: "面试反馈",
    no_scorecard_filled: "暂未填写面试评分卡",
    pending_evaluation_signals: "待评估信号",
    other_observed_signals: "其它观察到的信号",
    other_observed_signals_hint:
      "用于记录观察到的、不属于主要评估信号的重要行为",
    main_concerns_red_flags: "主要顾虑/红线问题",
    main_concerns_red_flags_hint: "对观察到的最重要风险或负面信号的总结",
    round_suffix: "面",
    update_failed: "更新失败",

    // 子组件相关翻译
    ideal_profile: {
      minimum: "最低要求",
      big_plus: "重要加分项",
      plus: "加分项",
      strengths: "优势",
      potential_gaps: "潜在差距",
      career_motivations: "职业动机",
    },
    feedback_summary: {
      recommend_next_stage: "推荐进入下一阶段",
      keep_watching: "保持观望",
      not_recommend: "不予推荐",
      overall_recommendation: "总体推荐",
      round_summary: "本轮小结",
      next_round_operational_suggestions: "下一轮可操作性建议",
      interviewer: "面试官",
      update_success: "更新成功",
      copied: "已复制",
      chat_with_viona: "与 Viona 对话",
      submit: "提交",
      cancel: "取消",
    },
    feedback_signal: {
      exceeds_expectations: "超出预期",
      meets_standards: "达标",
      likely_meets: "大概率达标",
      likely_does_not_meet: "大概率不达标",
      does_not_meet: "不达标",
      uncertain: "不确定",
      not_assessed: "本次面试未涉及",
      evaluation_result: "评估结果",
      basis: "依据",
      save: "保存",
      cancel: "取消",
      edit: "编辑",
      evidence: "证据",
      no_data: "暂无数据",

      // 经验背景适配性评估选项
      over_qualified: "资历过高/过于资深",
      more_senior: "比要求更资深",
      meets_seniority_bar: "符合资历要求",
      slightly_junior: "略微初级",
      too_junior: "过于初级",
      uncertain_not_assessed: "不确定/未评估",

      // 关键职责评估选项
      directly_relevant: "直接相关经验",
      highly_transferable: "高度可迁移经验",
      partially_transferable: "部分可迁移经验",
      no_relevant: "无相关经验",

      // 工作环境评估选项
      identical_environment: "相同环境",
      similar_environment: "相似环境",
      different_environment: "不同环境",
    },
    feedback_customize_signal: {
      judgment_basis: "判断依据",
      save: "保存",
      cancel: "取消",
      edit: "编辑",
      evidence: "证据",
      no_data: "暂无数据",
    },

    // 信号分组映射相关翻译
    group_mapping: {
      part1: {
        title: "第一部分：经验与情境匹配度",
        experience_contextual_fit: "1.1 综合资深程度要求",
        key_responsibilities: "1.2 候选人是否具备履行我们关键职责的成功记录？",
        working_environment:
          "1.3 候选人是否具备在类似我们这样的工作环境中茁壮成长的成功记录？",
      },
      part2: {
        title: "第二部分：技能、领域知识与专业资产",
        skills: "2.1 技能",
        domain_knowledges: "2.2 领域知识",
      },
      part3: {
        title: "第三部分：个人特质",
      },
      part4: {
        title: "第四部分：教育背景与认证",
      },
      part5: {
        title: "第五部分：物流与杂项",
      },
    },
  },

  city_and_address_select: {
    city: "城市",
    address: "地址(非必填)",
  },

  manager_detail: {
    job_title: "直属领导的职位名称是什么？",
    name: "直属领导姓名",
  },

  job_requirement: {
    name: "姓名",
    company_name: "公司",
    job_title: "职位",
    email: "邮箱",
    phone: "手机号",
    job_name: "职位名称",
  },

  chatbot_config: {
    modal_title: "聊天机器人配置",
    allow_salary: "您希望 Viona 如何与候选人沟通薪酬方面的信息？",
    not_permitted: "完全禁止",
    not_permitted_hint: "Viona 不得与候选人讨论任何薪酬细节。",
    structure_only: "仅限结构/组成部分",
    structure_only_hint:
      "可讨论薪酬包的构成要素（如底薪、奖金、期权、福利等），但不提供任何具体数字或薪酬范围。",
    structore_and_range: "结构 + 大致范围",
    structore_and_range_hint:
      "可讨论薪酬结构和组成部分，并提供与该职位相关的大致薪酬范围或近似数字。",
    specific_details: "具体细节",
    specific_details_hint:
      "可讨论具体的薪酬数字，包括精确的数字或针对该职位的具体/目标薪酬范围。",
    others:
      "关于 Viona 与候选人就此职位进行沟通，是否有任何其他指示？（例如：希望 Viona 避免谈论的内容、需要特别强调的方面等）",
  },

  talents: {
    status_evaluate_succeed: "待审核",
    status_accepted: "已通过",
    status_rejected: "已拒绝",
    assigned_recruiter_placeholder: "指定 Recruiter",
  },

  interview_designer: {
    title: "面试问题",
    new_chat: "创建",
    round: "第{{round}}轮",
    chat: "对话",
    edit_context: "编辑上下文",
    resume: "候选人简历",
    round_field: "面试轮次",
    last_round_feedback: "上轮反馈",
  },

  interview_feedback: {
    title: "面试反馈",
    new_chat: "创建",
    round: "第{{round}}轮",
    chat: "对话",
    edit_context: "编辑上下文",
    resume: "候选人简历",
    round_field: "面试轮次",
    interview_transcript: "面试对话",
  },

  ...CandidateResume,

  public_jobs: {
    banner: {
      title: "大标题",
      subTitle: "副标题",
      search: {
        placeholder: "搜索职位、公司名称、地点",
        hotSearch: "热门搜索",
        hotSearchTags: [
          "AI工程师",
          "UI设计师",
          "前端开发",
          "数据分析师",
          "测试",
        ],
      },
    },
    job_recommendations: {
      title: "岗位推荐",
    },
    job_card: {
      team_language: "团队语言:",
      role_type: {
        onsite: "完全在办公室工作",
        hybrid: "混合型",
        remote: "完全在家工作",
      },
      employee_level: {
        internship: "实习生",
        no_experience: "应届毕业生/无经验",
        junior: "初级/少量经验",
        mid_level: "中级/有一定经验",
        senior: "高级/经验非常丰富",
      },
      time_ago: {
        minute: "分钟前",
        hour: "小时前",
        day: "天前",
      },
    },
    sidebar: {
      login_register: {
        title: "登录/注册账号",
        hint: "聊一聊完善简历，提升求职成功率",
        button: "登录/注册",
      },
      company_ranking: {
        title: "热门企业排行榜",
      },
    },
  },

  home_header: {
    candidates: "我要找人",
    employers: "我要招聘",
    home: "首页",
    jobs: "职位",
    recruit: "我要招聘",
    jobseeker: "我要找人",
    login_register: "登录/注册",
  },

  signup: {
    title: "注册账号",
    email: "公司邮箱",
    password: "密码",
    confirm_password: "确认密码",
    verification_code: "验证码",
    your_name: "您的姓名",
    position: "职位",
    phone: "手机号",
    company_name: "公司名称",
    website: "网站",
    next_step: "下一步",
    sign_up: "注册",
    already_have_account: "已有账号？",
    sign_in: "登录",
    send_code: "发送验证码",
    sending: "发送中...",
    countdown: "秒",
    email_placeholder: "请输入邮箱",
    password_placeholder: "请输入密码",
    confirm_password_placeholder: "请确认密码",
    verification_code_placeholder: "请输入验证码",
    name_placeholder: "请输入您的姓名",
    position_placeholder: "请输入您的职位",
    phone_placeholder: "请输入手机号",
    company_placeholder: "请输入公司名称",
    website_placeholder: "请输入网站",
    please_enter_email: "请输入邮箱",
    please_enter_password: "请输入密码",
    please_confirm_password: "请确认密码",
    please_enter_verification_code: "请输入验证码",
    please_enter_name: "请输入您的姓名",
    please_enter_position: "请输入您的职位",
    please_enter_phone: "请输入手机号",
    please_enter_company_name: "请输入公司名称",
    please_enter_website: "请输入网站",
    password_length_error: "密码长度需不少于8位",
    password_uppercase_error: "密码需包含大写字母",
    password_lowercase_error: "密码需包含小写字母",
    password_number_error: "密码需包含数字",
    password_special_error: "密码需包含特殊字符",
    confirm_password_error: "确认密码不正确",
    phone_format_error: "请输入有效的手机号（支持国际号码）",
    verification_code_sent: "验证码已发送",
    verification_code_failed: "验证码发送失败",
    verification_code_incorrect: "验证码不正确",
    signup_succeed: "注册成功",
    signup_failed: "注册失败",
    email_exists: "邮箱已存在",
    basic_info_title: "提供您的基本信息",
    company_info_title: "提供您的公司信息",
    recruitment_requirement_title: "提供您的招聘需求",
    role_type: "职位类型",
    headcount_number: "招聘人数",
    please_enter_role_type: "请选择职位类型",
    please_enter_headcount_number: "请选择招聘人数",
    headcount_number_placeholder: "请选择招聘人数",
    prev: "上一步",
    role_type_options: {
      "engineering/tech": "工程/技术",
      "data/ai": "数据/AI",
      product: "产品",
      "design/ux": "设计/UX",
      sales: "销售",
      "marketing/growth": "营销/增长",
      "customer success/support": "客户成功/支持",
      operations: "运营",
      finance: "财务",
      "hr/people": "人力资源/人事",
      "legal/compliance": "法律/合规",
      "it/security": "IT/安全",
      "supply chain/procurement": "供应链/采购",
      manufacturing: "制造",
      facilities: "设施",
      "medical/clinical": "医疗/临床",
      other: "其他",
    },
    company_size: "公司规模",
    please_enter_company_size: "请选择公司规模",
    company_size_options: {
      lte_10: "1-10人",
      "11_to_50": "11-50人",
      "51_to_100": "51-100人",
      "101_to_500": "101-500人",
      "501_to_1000": "501-1000人",
      gte_1001: "1000+人",
    },
    signin_with_google: "使用 Google 登录",
    signup_with_email: "使用邮箱注册",
    agree_terms_warning: "请同意服务条款和隐私政策",
    agree_terms_text: "登录即表示您同意服务条款和隐私政策",
  },

  signin: {
    title: "登录",
    email: "邮箱",
    password: "密码",
    sign_in: "登录",
    dont_have_account: "还没有账号？",
    sign_up: "注册",
    forgot_password: "忘记密码？",
    please_enter_email: "请输入邮箱",
    please_enter_password: "请输入密码",
    email_placeholder: "请输入邮箱",
    password_placeholder: "请输入密码",
    signin_succeed: "登录成功",
    username_or_password_incorrect: "用户名或密码不正确",
    deactivated_account: "该账号已停用",
  },

  staffPasswordReset: {
    forgotTitle: "忘记密码",
    resetTitle: "重置密码",
    forgotSubmitOk: "若该邮箱已注册为员工账号，您将收到重置密码邮件",
    resetOk: "密码已重置",
    newPassword: "新密码",
    confirmPassword: "确认密码",
    pleaseConfirm: "请再次输入新密码",
    passwordMismatch: "两次输入的密码不一致",
    minPassword: "密码至少 6 位",
    submitForgot: "发送重置邮件",
    resendAfter: "{{seconds}} 秒后可再次发送",
    tooFrequent: "请求过于频繁，请 1 分钟后再试",
    submitReset: "确认重置",
    backToSignin: "返回登录",
    goForgot: "重新申请重置",
    invalidEmail: "请输入有效邮箱",
    invalidParams: "请求参数有误",
    deactivatedAccount: "该账号已停用",
    requestFailed: "请求失败",
    tryLater: "请稍后重试",
    invalidOrExpired: "链接无效或已过期",
    missingToken: "重置链接无效，请从邮件重新打开或重新申请。",
    resetFailedTryLater: "重置失败，请稍后重试",
  },

  app_layout: {
    delete_job: "删除职位",
    delete_job_confirm:
      "确定要删除【{{jobName}}】吗？ 删除后原有内容无法恢复。",
    search_placeholder: "请输入",
    loading: "加载中...",
    expand_sidebar: "打开边栏",
    collapse_sidebar: "收起边栏",
    view_all: "查看全部",
  },

  dashboard: {
    title: "Dashboard",
    subtitle: "活跃职位与候选人 pipeline 概览",
    application_date: "申请日期：",
    candidate_scope: "候选人范围：",
    filter_all: "全部",
    filter_excl_rejected: "不含 Rejected",
    filter_only_rejected: "仅 Rejected",
    fetch_failed: "加载 Dashboard 数据失败",
    open_jobs: "Open Jobs",
    open_jobs_tooltip: "对你可见的已发布职位，不受申请日期筛选影响。",
    pending_intake: "Pending Intake",
    applications: "Applications",
    responded: "AI Screening Responded",
    responded_tooltip: "含进行中与已完成",
    completed: "AI Screening Completed",
    results: "AI Screening Results",
    results_tooltip: "仅含 Completed",
    maybe_or_above: '"Maybe" 及以上',
    rejected: "Rejected",
    never_started: "Never Started",
    in_progress: "In Progress",
    of_applications: "占 Applications",
    of_applications_with_symbol: "占 Applications 的",
    of_responded: "占 Responded",
    of_responded_with_symbol: "占 Responded 的",
    trends: "Trends",
    granularity_daily: "按日",
    granularity_weekly: "按周",
    granularity_monthly: "按月",
    trends_empty: "请至少选择一项指标",
    by_team: "Applications by Team",
    by_job: "Applications by Job",
    by_job_hint: "（最近发布的 Top 10）",
    filter_departments: "筛选部门",
    filter_departments_placeholder: "选择部门",
    team_direct_tooltip: "仅统计直接挂在本 team 下的职位，不含子 team。",
    col_team: "Team",
    col_job: "Job",
    col_open_jobs: "Open Jobs",
    col_ai_screening: "AI Screening",
    col_applications: "Applications",
    col_responded: "Responded",
    col_completed: "Completed",
    col_maybe_plus: "Maybe 及以上",
    col_active: "Active",
    col_rejected: "Rejected",
    col_last_posted: "最近发布",
    days_open: "开放 {{count}} 天",
    teams_footnote: "显示第 {{start}}–{{end}} 条，共 {{total}} 个 team",
    pipeline_distribution: "Pipeline Distribution",
    active_applications: "(Active Applications)",
    active_applications_tooltip: "未被拒绝的申请",
    col_stage: "Stage",
    col_of_applications: "# of Applications",
    col_pct_of_active: "% of Active",
    col_median_days: "Median Days in Stage",
    col_p90_days: "P90 Days in Stage",
  },

  job_list: {
    title: "职位列表",
    search_placeholder: "请输入职位名称进行搜索",
    owner_placeholder: "负责人",
    recruiter_placeholder: "招聘专员",
    create_job: "创建职位",
    details: "详情",
    go_to_listing: "查看职位页面",
    columns: {
      id: "ID",
      job_title: "职位名称",
      post_status: "发布状态",
      owner: "负责人",
      recruiters: "招聘专员",
      post_time: "发布时间",
      total_candidates: "候选人总数",
      candidates_passed_screening: "通过筛选的候选人",
      actions: "操作",
    },
    post_status: {
      published: "已发布",
      unpublished: "未发布",
      delisted: "已下架",
    },
    empty: {
      description_line_1: "在这里发布你的第一个职位，",
      description_line_2: "我们会为你匹配最合适的顶尖人才",
      alt_text: "职位列表为空",
    },
  },

  job_details: {
    updated_at: "更新",
    job_requirements: "职位需求",
    pipeline: "Pipeline",
    pipeline_section: {
      search_placeholder: "搜索候选人...",
      all_sourcing_channels: "全部招聘渠道",

      all_stages: "全部阶段",
      all_fit_levels: "全部匹配度",
      rank_by: "排序",
      list: "列表",
      kanban: "看板",
      in_batch: "批量操作",
      add_candidate: "添加候选人",
      candidates_reached_out: "位候选人已触达",
      click_to_view_details: "点击查看详情",
      persevio: "Persevio",
      candidate_name: "候选人姓名",
      stage: "阶段",
      overall_fit: "综合匹配度",
      assigned_recruiters: "指定 Recruiter",
      experience: "经验",
      visa_status: "签证状态",
      current_comp: "当前薪资",
      days_in_stage: "阶段天数",
      move_stage: "移动阶段",
      schedule_interview: "安排面试",
      view_interview_info: "查看面试信息",
      reject: "拒绝",
      move_stage_modal_title: "移动阶段",
      fit_high: "高",
      fit_medium: "中",
      fit_low: "低",
      actions: "操作",
      upload_candidate: "上传候选人",
      upload_candidate_modal_title: "上传候选人",
      resume_label: "简历",
      resume_upload_hint: "支持 .doc、.docx、.pdf，与候选人注册流程一致",
      uploading_resume: "正在上传并解析简历...",
      resume_upload_success: "简历解析成功",
      parse_resume_failed: "解析简历失败",
      please_upload_resume_first: "请先上传简历",
      field_name: "姓名",
      field_email: "邮箱",
      field_phone: "手机号",
      field_required: "请填写此项",
      field_placeholder: "请填写",
      field_email_invalid: "请输入有效邮箱",
      field_phone_invalid: "请选择区号并填写手机号",
      field_phone_format_invalid: "请输入有效的手机号",
      field_source_channel: "来源渠道",
      field_source_channel_placeholder: "请选择来源渠道",
      field_source_channel_required: "请选择来源渠道",
      field_notify_interview: "邀请候选人进行 AI 面试",
      cancel: "取消",
      submit_create_candidate: "创建候选人",
      create_candidate_success: "候选人创建成功",
      create_candidate_failed: "创建候选人失败",
      show_unsuitable: "显示 {{count}} 个不匹配",
      hide_unsuitable: "隐藏 {{count}} 个不匹配",
      please_select_talents: "请选择候选人以继续",
      selected_talents_already_rejected:
        "未采取任何操作 - 所有选中的候选人已处于拒绝状态",
    },
    sourcing_channels: "招聘渠道",
    sourcing_channels_section: {
      free_job_board_title: "Persevio 免费招聘看板",
      free_job_board_desc: "您的职位已发布到 Persevio 招聘看板，完全免费。",
      copy_url: "复制链接",
      view_posting: "查看发布",
      take_down: "撤下",
      outreach_title: "外展活动",
      outreach_desc:
        "针对未积极寻找工作的被动候选人。我们将主动联系 Persevio 数据库和全网中合适的候选人，利用推荐和所有可能的渠道为您的职位找到最佳人选。按成功收费——仅在您招聘成功时支付。",
      start_outreach_campaigns: "开始外展活动",
      outreach_modal_title: "Start Outreach Campaigns",
      outreach_modal_desc:
        "With Outreach Campaigns, Persevio will proactively source and engage passive candidates who aren't actively job-seeking:",
      outreach_modal_point_1:
        "Search and identify suitable candidates from Persevio's talent database",
      outreach_modal_point_2:
        "Proactive outreach via email, LinkedIn, and professional networks",
      outreach_modal_point_3: "Leverage referrals and industry connections",
      outreach_modal_point_4:
        "Screen and present qualified candidates directly to your pipeline",
      outreach_modal_pricing_title: "Pricing",
      outreach_modal_pricing_desc:
        "Success-based pricing — you only pay when you successfully hire a candidate sourced through outreach. Our team will discuss the specific fee structure for your role.",
      outreach_modal_contact_sales: "Contact Sales",
      tracking_links_title: "追踪链接",
      tracking_links_desc:
        "将您在其他招聘板上的帖子中的候选人引导至下方相应的链接，以便所有候选人将在系统中聚合。",
      source: "来源",
      url: "URL",
      actions: "操作",
      copy: "复制",
      add_custom_source: "添加自定义来源",
      source_name: "渠道名称",
      source_name_placeholder: "e.g. Indeed",
      generate_url: "生成 URL",
      cancel: "取消",
      delete: "删除",
      delete_custom_source_title: "删除自定义渠道？",
      delete_custom_source_content:
        "确定要删除「{{name}}」吗？此操作无法撤销。",
    },
    outreach_campaigns: "外推活动",
    outreach_campaigns_table: {
      candidate_name: "候选人姓名",
      current_job_title: "当前职位",
      current_company: "当前公司",
      status: "状态",
      reached_out: "触达时间",
      last_activity: "最近活动",
      all_statuses: "全部状态",
      reached_out_status: "已触达",
      viewed_job_info_status: "已查看职位",
      resume_submitted_status: "已投递简历",
    },
    dashboard: "Dashboard",
    analytics: "数据分析",
    analytics_section: {
      all_sources: "全部来源",
      all_time: "全部时间",
      last_30_days: "最近 30 天",
      pipeline_funnel: "招聘漏斗",
      detailed_stage_breakdown: "阶段明细",
      stage: "阶段",
      candidates: "候选人",
      conversion_rate: "转化率",
      conversion_rate_label: "转化率",
      avg_days_in_stage: "阶段平均停留天数",
      top_source: "主要来源",
      top_source_persevio_outreach: "Persevio Outreach",
      starting: "起始",
    },
    job_requirement_table: "职位需求表",
    job_description_jd: "职位描述(JD)",
    internal: "Internal",
    download_pdf: "Download PDF",
    edit: "编辑",
    job_requirement_hint:
      "Viona 会参考这份文档向候选人介绍该职位。你可以在此编辑内容，以控制她与候选人沟通时的说法。",
    talents: "候选人",
    settings: "设置",
    share_position: "分享职位",
    copy_document: "复制文档",
    unpost_job_title: "取消发布职位？",
    unpost_job_content:
      "取消发布后，职位将不再对候选人可见。您可以随时重新发布。",
    unpost_job: "取消发布",
    post_job_title: "发布职位？",
    post_job_content: "发布后，职位将对候选人可见。您可以随时取消发布。",
    post_job: "发布",
    job_posting: "职位页面",
    conversation_record: "对话记录",
    edit_with_stars: "编辑",
    chat_with_viona_menu: "与 Viona 对话",
    manually_edit: "手动编辑",
    refining_job_requirements: "完善职位需求",
    jrd_edit_chat_failed: "无法打开 JRD 对话，请稍后重试。",
    publish_job_hint:
      "请先将职位取消发布，再编辑修改JD内容，修改完成后方可重新发布。",
    assignHunters: "分配猎头",
    save: "保存",
    job_collaborator_modal: {
      description:
        "请先选择当前职位的协作者，点击【更新】按钮确认，再点击【复制职位链接】，将链接发给对应的协作者。",
      title: "职位协作者",
      select_collaborators: "选择协作者",
      select_placeholder: "请选择协作者",
      update: "更新",
      copy_link: "复制职位链接",
    },
  },

  job_settings: {
    pipeline_stages: "Pipeline Stages",
    pipeline_stages_desc:
      "Drag to reorder stages. The first 4 stages are locked and cannot be moved or deleted.",
    add_stage: "Add Stage",
    loading: "加载中...",
    delete_stage_title: "删除阶段",
    delete_stage_content: "确定要删除阶段「{{name}}」吗？",
    unnamed_stage: "未命名",
    org_node: "所属组织/部门",
    org_node_placeholder: "不选择则不归属",
    apply_inbound_email: "虚拟邮箱（接收该 Job 的邮件）",
    copy: "复制",
    copy_success: "已复制",
    copy_failed: "复制失败",
    collaborators: "协作者",
    add_collaborator_placeholder: "添加协作者...",
    role_owner: "负责人",
    role_recruiter: "招聘专员",
    role_hiring_manager: "用人经理",
    add: "添加",
    remove: "移除",
    remove_collaborator_title: "移除协作者",
    remove_collaborator_content: "确定要移除协作者「{{name}}」吗？",
    activity: "Activity",
    created_by_prefix: "Created by",
    created_on: "on",
    view_activity_logs: "View Activity Logs",
    activity_logs_title: "Job Activity Logs",
    activity_log_time: "Time",
    activity_log_action_col: "Action",
    activity_log_description_col: "Description",
    activity_log_operated_by: "Operated by",
    activity_log_confidential_yes: "Yes",
    activity_log_confidential_no: "No",
    activity_log_action: {
      create: "Create",
      delist: "Delist",
      publish: "Publish",
      pipeline_stage_add: "Pipeline Stage - Add",
      pipeline_stage_rename: "Pipeline Stage - Rename",
      pipeline_stage_remove: "Pipeline Stage - Remove",
      pipeline_stage_reorder: "Pipeline Stage - Reorder",
      collaborator_add: "Collaborator - Add",
      collaborator_edit: "Collaborator - Edit",
      collaborator_remove: "Collaborator - Remove",
      confidentiality_set: "Confidentiality Set",
      custom_source_add: "Custom Source - Add",
      jrd_edit_ai: "JRD - Edit",
      jrd_edit_manual: "JRD - Edit",
      jd_edit_manual: "JD - Edit",
    },
    activity_log_description: {
      create: "Created this Job",
      delist: "Delisted this Job",
      publish: "Published this Job",
      pipeline_stage_add: 'Stage "{{stage_name}}" added',
      pipeline_stage_rename:
        'Stage "{{old_stage_name}}" renamed to "{{new_stage_name}}"',
      pipeline_stage_remove: 'Stage "{{stage_name}}" removed',
      pipeline_stage_reorder:
        'Display order of Stage "{{stage_name}}" adjusted',
      collaborator_add: '"{{name}}" added as {{role}}',
      collaborator_edit: '"{{name}}" changed to {{role}}',
      collaborator_remove: '"{{name}}" removed',
      confidentiality_set: '"Is Confidential" set to {{value}}',
      custom_source_add: 'Custom Source "{{source_name}}" added',
      jrd_edit_ai: "JRD updated via AI chat",
      jrd_edit_manual: "JRD updated manually",
      jd_edit_manual: "JD updated manually",
    },
  },

  talent_details: {
    confirm_accept: "确定要通过该候选人吗？",
    confirm_reject: "确定要拒绝该候选人吗？",
    status_accepted: "已通过",
    status_rejected: "未通过",
    action_accept: "通过",
    action_reject: "拒绝",
    download_resume: "下载简历",
    ai_interview_record: "AI 面试记录",
    candidate_evaluation_report: "候选人评估报告",
    update_success: "更新成功",
    tab_resume: "简历",
    tab_report: "报告",
    no_report: "暂无报告",
    schedule_interview: "安排面试",
    interview_scheduled: "已安排面试",
    awaiting_candidate_confirm: "待候选人确认面试详情",
    reject_candidate_title: "拒绝候选人",
    reject_candidate_title_with_name: "拒绝候选人 — {{name}}",
    reject_candidates_batch_title: "批量拒绝候选人",
    reject_reason_label: "请告诉我们为什么该候选人不合适，以便我们调整搜索",
    reject_reason_required: "请输入原因",
    reject_reason_type_label: "原因",
    reject_reason_not_shortlisted: "未入围",
    reject_reason_failed_interview: "面试未通过",
    reject_reason_headcount_freeze: "编制冻结",
    reject_reason_candidate_withdrew: "候选人退出",
    reject_reason_others: "其他",
    reject_feedback_prompt:
      "为了更好地为该职位寻访与筛选候选人，我们还需要了解哪些信息？",
    reject_feedback_placeholder:
      "例如：评估不够准确 / 职位要求已变化希望调整 JD / 其他任何反馈。",
    reject_notes_optional_label: "备注（选填）",
    reject_reason_details_label: "原因详情",
    reject_notes_placeholder: "如有需要可补充说明",
    reject_batch_notes_placeholder: "适用于所有已选候选人 — 如有需要可补充说明",
    reject_reason_details_placeholder: "请简要说明",
    reject_calibration_guidance:
      "我评估该候选人通过。您显然不同意。请帮助我理解我哪里错了，以便我将来能更准确地为您筛选候选人。",
    reject_calibration_placeholder:
      "例如：支付系统实战经验不足 / 职位要求已变化，需要更强的 leadership 背景……",
    action_reject_only: "仅拒绝",
    action_reject_and_chat: "拒绝并与 Viona 对话",
    action_reject_all: "全部拒绝",
    reject_submit_failed: "提交失败",
    batch_reject_success: "拒绝成功：{{count}}",
    batch_reject_result_rejected: "已拒绝：{{count}}",
    batch_reject_result_skipped: "已跳过：{{count}}（已是拒绝状态）",
    batch_reject_result_failed: "失败：{{count}}",
    jobs_applied: "已投职位",
    applications_count: "{{count}} 份申请",
    applied_meta: "申请日期：{{date}} | 来源：{{source}}",
    tab_interview_feedback_count: "面试反馈",
    tab_notes_count: "备注 ({{count}})",
    tab_activity: "动态",
    tab_assigned_recruiters: "指定 Recruiter",
    assign_recruiter_placeholder: "指定 Recruiter...",
    no_assigned_recruiters: "暂无指定 Recruiter",
    new_interview_round: "+ 新建轮次",
    download_report_english: "English",
    download_report_chinese: "Chinese (Simplified)",
    download_report_native: "Native",
    download_report_ai_translated: "AI Translated",
    download_report_translating: "正在翻译报告...",
    download_report_translate_failed: "报告翻译失败",
    download_resume_in: "下载简历语言",
    download_resume_original: "原文",
    download_resume_other_language: "其它语言",
    download_resume_translating: "正在翻译简历...",
    download_resume_translate_failed: "简历翻译失败",
    view_resume_in: "查看简历语言",
    switch_resume_language: "切换简历语言",
    looks_accurate: "符合预期",
    looks_inaccurate: "不符合预期",
  },

  admin_talents: {
    candidate_name: "候选人姓名",
    company_name: "公司名称",
    job_name: "职位名称",
    current_job_title: "当前职位",
    current_company: "当前公司",
    current_compensation: "当前薪资",
    expected_compensation: "期望薪资",
    visa: "签证",
    received_on: "申请时间",
    account_status: "账号状态",
    screening_status: "申请状态",
    schedule_time: "面试时间",
    actions: "操作",
    header: "候选人",
    rejectReason: "拒绝原因",
    interviewMode: "面试模式",
    viewDetails: "查看详情",
    editHireStatus: "编辑雇佣状态",
    viewReferralLinkDetails: "查看推荐链接详情",
    hire_status: "雇佣状态",
    hire_status_options: {
      hired: "已雇佣",
      not_hired: "未雇佣",
    },
    operationSuccess: "操作成功",
    operationFailed: "操作失败",
    account_status_options: {
      evaluated: "已评估",
      message_generated: "已生成消息",
      message_sent: "已发送消息",
      message_read: "已读消息",
      registered: "已填基本信息",
      resume_uploaded: "已上传简历",
      email_binded: "已绑定邮箱",
    },
    screening_status_options: {
      initialize: "未申请",
      interviewing: "面试中",
      interview_finished: "猎头审核中",
      hunter_rejected: "猎头已拒绝",
      hunter_accepted: "雇主审核中",
      staff_rejected: "雇主已拒绝",
      staff_accepted: "雇主已通过",
      interview_scheduled: "已安排面试",
      interview_confirmed: "已确认面试",
    },
    drawer: {
      outreachMessageReport: "外推消息报告",
      accept: "接受",
      reject: "拒绝",
      accepted: "已接受",
      rejected: "已拒绝",
      basicInfo: "基本信息",
      interviewModeLabel: "面试模式:",
      selectHumanReasonLabel: "选择人工原因:",
      employerRejectReasonLabel: "雇主未通过原因:",
      employerInterviewModeLabel: "雇主面试模式:",
      employerInterviewTimeLabel: "雇主面试时间:",
      interviewTypeLabel: "面试形式:",
      jd: "JD",
      recommendReport: "推荐报告",
      editReport: "编辑报告",
      viewResume: "查看简历",
      downloadResume: "下载简历",
      viewChat: "查看对话",
      noReport: "暂无报告",
      resume: "简历",
      chat: "对话",
      outreachMessage: "Outreach Message",
      notGenerated: "未生成",
      messageSentTime: "消息发送时间",
      messageReadTime: "消息已读时间",
      recommendationReport: "推荐报告",
    },
    hireStatusModal: {
      title: "雇佣状态",
      cancel: "取消",
      updateSuccess: "更新雇佣状态成功",
      updateFailed: "更新雇佣状态失败",
      description:
        "如果被推荐人已被公司雇佣，推荐链中的所有用户将分享 {{bonusPool}} S$；请及时联系用户。",
      label: "请选择被推荐人当前的雇佣状态",
      hired: "已雇佣",
      notHired: "未雇佣",
    },
    referralChainModal: {
      title: "推荐链详情",
    },
  },

  candidate_list_page: {
    title: "候选人列表",
    all_jobs: "全部职位",
    columns: {
      candidate: "候选人",
      last_job_title: "简历职位",
      last_company: "简历公司",
      applied_for: "投递职位",
      recruiters: "招聘专员",
      experience: "经验",
      visa_status: "签证状态",
      current_comp: "当前薪资",
    },
  },

  scoped_talents: {
    pageTitle: "候选人列表",
    filters: {
      talentOrJobName: "候选人/职位名称:",
      talentOrJobNamePlaceholder: "按候选人/职位名称筛选",
      approveStatus: "申请状态:",
      approveStatusPlaceholder: "筛选申请状态",
      filter: "筛选",
      clear: "清空",
    },
  },

  admin_companies: {
    pageTitle: "公司列表",
    searchPlaceholder: "公司名称",
    fetchFailed: "获取公司列表失败",
    operationFailed: "操作失败",
    table: {
      id: "ID",
      companyName: "公司名称",
      registerEmail: "注册邮箱",
      website: "公司网址",
      registrantName: "注册人姓名",
      position: "职位名称",
      phone: "电话号码",
      companySize: "公司规模",
      roleType: "职位类型",
      headcount: "招聘名额",
      credits: "Credits",
      status: "审核状态",
      actions: "操作",
    },
    status: {
      approving: "审核中",
      rejected: "未通过",
      approved: "已通过",
    },
    buttons: {
      reject: "不通过",
      approve: "通过",
    },
    filters: {
      all: "所有",
      approving: "审核中",
      approved: "已通过",
      rejected: "未通过",
    },
    audit: {
      title: "审核企业",
      confirmApprove: "确定要通过该企业吗？",
      confirmReject: "确定要拒绝该企业吗？",
      approveSuccess: "已通过该公司",
      rejectSuccess: "已拒绝该公司",
    },
    pagination: {
      total: "第 {{start}}-{{end}} 条，共 {{total}} 条",
    },
    creditsDrawer: {
      title: "Credits · {{name}}",
    },
  },

  member_team: {
    title: "成员与团队",
    membersTitle: "成员",
    membersSubtitle: "{{name}} · 共 {{count}} 人",
    membersEmpty: "该团队范围内暂无成员",
    searchPlaceholder: "成员姓名、账号邮箱",
    createMember: "创建成员",
    editMember: "编辑成员",
    team: "团队",
    empty: "暂无组织节点数据",
    addChild: "添加子节点",
    rename: "重命名",
    delete: "删除",
    addChildModalTitle: "添加子节点",
    renameModalTitle: "重命名节点",
    nodeName: "节点名称",
    nodeNamePlaceholder: "请输入名称",
    nodeNameRequired: "请输入名称",
    confirm: "确定",
    cancel: "取消",
    teamCreateSuccess: "已创建",
    teamCreateFailed: "创建失败",
    renameSuccess: "已更新名称",
    renameFailed: "重命名失败",
    deleteTitle: "删除节点",
    deleteConfirm: "确定删除「{{name}}」？仅允许删除无子节点且无引用的节点。",
    deleteSuccess: "已删除",
    deleteFailed: "删除失败",
    teamUpdateSuccess: "排序已更新",
    teamUpdateFailed: "更新失败",
    cannotMoveRoot: "不能移动公司根节点",
    selectOrgNodeHint: "请在左侧选择一个组织节点",
    maxDepthExceeded: "部门层级超出最大值",
    id: "ID",
    name: "姓名",
    email: "邮箱",
    role: "角色",
    status: "状态",
    action: "操作",
    edit: "编辑",
    admin: "管理员",
    normal: "员工",
    recruiter: "招聘人员",
    hiring_manager: "用人经理",
    active: "启用中",
    deactivated: "已停用",
    activate: "启用",
    deactivate: "停用",
    orgNode: "组织位置",
    orgNodePlaceholder: "可选，选择所在组织节点",
    orgNodeRequired: "请选择一个团队",
    visibilityScope: "可见性范围（组织）",
    visibilityPlaceholder: "可选，选择可查看的组织范围",
    update: "更新",
    namePlaceholder: "请输入员工姓名",
    nameRequired: "请输入姓名",
    emailPlaceholder: "请输入员工邮箱",
    emailRequired: "请输入邮箱",
    emailInvalid: "请输入有效的邮箱格式",
    rolePlaceholder: "请选择角色",
    roleRequired: "请选择角色",
    passwordLabel: "默认密码",
    passwordRequired: "密码不能为空",
    copyLoginInfo: "复制登录信息",
    successMessage: "账号创建成功。可复制登录信息，发送给用户登录。",
    copySuccess: "账号信息已复制到剪贴板",
    staffExists: "员工已存在",
    memberCreateFailed: "员工创建失败",
    memberUpdateSuccess: "员工信息更新成功",
    memberUpdateFailed: "员工信息更新失败",
    deactivateConfirmTitle: "确认停用 {{name}} 吗？",
    deactivateConfirmDesc: "该账号将无法登录并无法继续使用相关功能。",
    activateSuccess: "账号已启用",
    deactivateSuccess: "账号已停用",
    toggleStatusFailed: "操作失败，请稍后重试",
    loginInfoTemplate:
      "登录链接: {{- loginUrl}}\n账号名称: {{- name}}\n账号邮箱: {{- email}}\n默认密码: {{- password}}",
  },

  groups: {
    title: "分组列表",
    searchPlaceholder: "请输入分组名称",
    createGroup: "创建分组",
    editGroup: "编辑分组",
    name: "名称",
    staffCount: "员工数量",
    staffIds: "员工",
    action: "操作",
    edit: "编辑",
    delete: "删除",
    update: "更新",
    confirm: "确认",
    cancel: "取消",
    namePlaceholder: "请输入分组名称",
    nameRequired: "请输入分组名称",
    staffPlaceholder: "请选择员工",
    createSuccess: "分组创建成功",
    createFailed: "分组创建失败",
    updateSuccess: "分组更新成功",
    updateFailed: "分组更新失败",
    deleteSuccess: "分组删除成功",
    deleteFailed: "分组删除失败",
    deleteTitle: "删除分组",
    deleteConfirm: "确认删除分组 {{name}}？删除后无法恢复。",
    deleteDisabledHint: "仅员工数量为 0 的分组可删除",
  },

  admin_candidates: {
    pageTitle: "用户列表",
    filters: {
      userNameLabel: "用户名称: ",
      userNamePlaceholder: "搜索用户名称",
    },
    table: {
      id: "ID",
      userName: "用户名称",
      email: "邮箱",
      createdAt: "注册时间",
      registerSource: "注册来源",
      status: "状态",
      resumeDetail: "简历详情",
      view: "查看",
    },
    status: {
      aiFinished: "已AI对话完",
      boundEmail: "已绑定邮箱",
      uploadedResume: "已上传简历",
      filledBasicInfo: "已填基本信息",
    },
    registerSource: {
      job: "职位",
      home: "首页",
    },
    error: {
      fetchFailed: "获取候选人列表失败",
    },
  },

  candidate_drawer_content: {
    profile: "个人资料",
    chatContent: "对话内容",
  },

  admin_jobs: {
    pageTitle: "职位列表",
    filters: {
      company: "公司",
      jobName: "职位名称",
      isPosted: "是否发布",
      filter: "筛选",
      clear: "清空",
      companyPlaceholder: "按公司筛选",
      jobNamePlaceholder: "按职位名称筛选",
      isPostedPlaceholder: "筛选是否发布",
    },
    table: {
      id: "ID",
      jobName: "职位名称",
      companyName: "公司名称",
      isPosted: "是否发布",
      applicationCount: "申请数量",
      employerCandidateCount: "猎头通过数量",
      passedScreeningCount: "雇主通过数量",
      confirmedInterviewCount: "已确认面试数量",
      bonusPool: "奖金池",
      assignee: "分配人",
      actions: "操作",
      published: "已发布",
      unpublished: "未发布",
      creating: "创建中",
      addBonusPool: "添加奖金池",
      goToListing: "查看职位页面",
    },
    recommendedCandidates: {
      id: "ID",
      candidate: "候选人",
      pushTime: "推送时间",
      recommendToCandidate: "推荐给候选人",
      recommendSuccess: "推荐成功",
    },
    bonusPoolModal: {
      title: "Add Bonus Pool",
      add: "Add",
      cancel: "Cancel",
      description:
        " Once the bonus pool amount is confirmed, this job will become eligible for recommendations. When a candidate is hired, all users in the referral chain will share the bonus pool.",
      label: "Please enter the total bonus pool amount for the current job",
      placeholder: "Input total amount",
      amountRequired: "Please enter the amount",
      addSuccess: "Add bonus pool success",
      addFailed: "Add bonus pool failed",
    },
  },

  admin_tools: {
    page_title: "管理工具",
    run_reminder_button: "手动触发 AI 面试提醒",
    run_weekly_report_button: "手动触发周报",
    run_monthly_report_button: "手动触发月报",
    confirm_title: "确认触发提醒",
    confirm_content: "确定要手动触发 AI 面试提醒吗？",
    confirm_weekly_report_title: "确认触发周报",
    confirm_weekly_report_content: "确定要手动触发周报吗？",
    confirm_monthly_report_title: "确认触发月报",
    confirm_monthly_report_content: "确定要手动触发月报吗？",
    run_reminder_success: "触发成功",
    run_reminder_failed: "触发失败",
    run_weekly_report_success: "周报触发成功",
    run_weekly_report_failed: "周报触发失败",
    run_monthly_report_success: "月报触发成功",
    run_monthly_report_failed: "月报触发失败",
  },

  admin_credit_configs: {
    pageTitle: "Credit Config",
    newPageTitle: "新建 Custom Config",
    editPageTitle: "编辑 Config",
    newCustomConfig: "新建 Custom Config",
    defaultSectionTitle: "Default Config",
    customSectionTitle: "Custom Configs",
    defaultType: "Default",
    customType: "Custom",
    inherit: "Inherited",
    override: "Overridden",
    loading: "加载中...",
    duplicateTitle: "复制 Config",
    duplicatePlaceholder: '例如 "VIP Customers (Copy)"',
    deleteConfirm: "确认删除该 Config？",
    unsavedTitle: "未保存的更改",
    unsavedContent: "你有未保存的修改，确定离开吗？",
    remarkPlaceholder: "备注（可选）",
    defaultAppliedHint: "All employers not assigned to any custom config",
    selectEmployers: "选择雇主",
    tabs: {
      configuration: "Configuration",
      history: "Edit History",
    },
    sections: {
      metadata: "Metadata",
      exchangeRates: "Exchange Rates",
      topupDefaults: "Topup Defaults",
      pricing: "Pricing",
    },
    fields: {
      name: "Config Name",
      type: "Type",
      currency: "Currency",
      appliedEmployers: "Applied to Employers",
      displayRate: "Display Rate",
      topupRate: "Topup Rate",
      topupValidity: "Topup Credit Validity",
      service: "Service",
      credits: "Credits",
      mode: "Inherit / Override",
    },
    services: {
      unlock_candidate_low_fit: "Unlock Candidate - Low fit",
      unlock_candidate_medium_fit: "Unlock Candidate - Medium fit",
      unlock_candidate_high_fit: "Unlock Candidate - High fit",
      unlock_candidate_hard_to_find_high_fit:
        "Unlock Candidate - Hard-to-find high fit",
      ai_screening: "AI Screening(per Application)",
      agency_placement: "Agency Placement",
    },
    validityMode: {
      never: "Never",
      "3_months": "3 months",
      "6_months": "6 months",
      "12_months": "12 months",
      "24_months": "24 months",
      custom: "Custom",
    },
    validityUnit: {
      days: "Days",
      months: "Months",
      years: "Years",
    },
    table: {
      id: "ID",
      name: "Name",
      appliedEmployers: "Applied to Employers",
      currency: "Currency",
      displayRate: "Display Rate",
      topupRate: "Topup Rate",
      modifiedTime: "Modified Time",
      modifiedBy: "Modified By",
      actions: "Actions",
    },
    filters: {
      configName: "Config Name",
      appliedCompany: "Applied to Company",
      selectCompany: "选择公司",
    },
    actions: {
      edit: "Edit",
      duplicate: "Duplicate",
      delete: "Delete",
      create: "Create",
      cancel: "Cancel",
      save: "Save",
      leave: "离开",
      stay: "留下",
    },
    messages: {
      fetchFailed: "加载 Credit Config 失败",
      saveSuccess: "保存成功",
      saveFailed: "保存失败",
      deleteSuccess: "删除成功",
      deleteFailed: "删除失败",
      duplicateSuccess: "复制成功",
      duplicateFailed: "复制失败",
    },
    validation: {
      nameRequired: "Config Name 必填",
      nameExists: "Config Name 已存在",
    },
    history: {
      empty: "暂无编辑历史",
      created: "Created config",
      deleted: "Deleted config",
      duplicated: "Duplicated config",
      duplicatedFrom: "Duplicated from config #{{id}}",
      updated: "Updated config",
      nameChanged: "Config name: {{before}} → {{after}}",
      fieldChanged: "{{label}}: {{before}} → {{after}}",
      addedEmployer: "Added employer: {{name}}",
      removedEmployer: "Removed employer: {{name}}",
      movedEmployer: "Moved employer: {{name}} from {{from}}",
    },
  },

  admin_credit_packages: {
    pageTitle: "Credit Package",
    createModalTitle: "发放 Credit Package",
    table: {
      id: "ID",
      company: "公司",
      originalAmount: "原始额度",
      remainingAmount: "剩余额度",
      validFrom: "生效时间",
      expiresAt: "过期时间",
      description: "描述",
      createdBy: "创建人",
      createdAt: "创建时间",
    },
    filters: {
      company: "公司",
      selectCompany: "选择公司",
    },
    form: {
      company: "公司",
      selectCompany: "选择公司",
      amount: "额度",
      sourceType: "来源类型",
      description: "描述",
      validFrom: "生效时间",
      expiresAt: "过期时间",
    },
    sourceType: {
      topup: "充值",
      gift: "赠送",
      adjustment: "调整",
    },
    actions: {
      create: "发放 Credit Package",
    },
    messages: {
      fetchFailed: "加载 Credit Package 失败",
      createSuccess: "发放成功",
      createFailed: "发放失败",
    },
    validation: {
      companyRequired: "请选择公司",
      amountRequired: "请填写额度",
      amountPositive: "额度必须为正整数",
    },
  },

  admin_mcf_jobs: {
    pageTitle: "MCF 职位",
    table: {
      jobPostId: "职位编号",
      title: "职位名称",
      status: "状态",
      company: "雇主",
      agency: "中介",
      salary: "薪资",
      ssocCode: "SSOC",
      category: "类别",
      region: "地区",
      newPostingDate: "最新发布",
      expiryDate: "到期日",
      views: "浏览",
      applications: "申请",
      onBehalf: "代发",
      yes: "是",
      no: "否",
      actions: "操作",
      view: "查看",
      openMcf: "打开 MCF",
    },
    filters: {
      search: "搜索",
      searchPlaceholder: "职位名称关键词",
      status: "状态",
      ssocCode: "SSOC 编码",
      addressRegion: "地区",
      category: "类别",
      employmentType: "雇佣类型",
      positionLevel: "职级",
      salaryMin: "最低薪资",
      companyUen: "雇主 UEN",
      companyName: "雇主名称",
      isPostedOnBehalf: "是否代发",
      newPostingDate: "最新发布日期",
      filter: "筛选",
      clear: "清空",
    },
    drawer: {
      titlePrefix: "MCF 职位",
    },
    messages: {
      fetchListFailed: "加载 MCF 职位失败",
    },
  },

  admin_kb_observations: {
    pageTitle: "KB Observation 列表",
    loading: "加载中...",
    table: {
      id: "ID",
      companyId: "公司 ID",
      chatId: "聊天 ID",
      entityType: "实体类型",
      entityId: "实体 ID",
      createdAt: "创建时间",
      actions: "操作",
      view: "查看",
    },
    drawer: {
      titlePrefix: "Observation 详情",
    },
    messages: {
      fetchListFailed: "加载 KB Observation 列表失败",
      fetchDetailFailed: "加载 KB Observation 详情失败",
    },
  },

  admin_kb_entity_documents: {
    pageTitle: "KB Entity Document 列表",
    loading: "加载中...",
    table: {
      id: "ID",
      companyId: "公司 ID",
      entityType: "实体类型",
      entityId: "实体 ID",
      createdAt: "创建时间",
      updatedAt: "更新时间",
      actions: "操作",
      view: "查看",
    },
    drawer: {
      titlePrefix: "Entity Document 详情",
    },
    tabs: {
      claim: "Claim",
      frontmatter: "Frontmatter",
      narrative: "Narrative",
    },
    messages: {
      fetchListFailed: "加载 KB Entity Document 列表失败",
      fetchDetailFailed: "加载 KB Entity Document 详情失败",
    },
  },

  interview_form: {
    create_success: "面试创建成功",
    job_company_name: "公司名称",
    job_title: "职位名称",
    interview_mode: "面试模式",
    select_mode_required: "请选择面试模式",
    written_test_link: "笔试链接",
    enter_link_required: "请输入笔试链接",
    interview_type: "面试类型",
    select_type_required: "请选择面试类型",
    interview_duration: "面试时长",
    enter_duration_required: "请输入面试时长",
    slots_gap: "时间段间隔",
    enter_gap_required: "请输入时间段间隔",
    interview_time_slots: "面试时间段",
    available_time_slots: "可用的面试时间段",
    select_slots_required: "请选择可用的时间段",
    time_slots_description:
      "请在右侧选择您的可用时间段，并为候选人提供多个选择。",
    interviewers: "面试官",
    interview_focus: "面试重点",
    contact_person: "联系人",
    contact_number: "联系电话",
    notes: "备注",
    mode_written: "笔试",
    mode_interview: "面试",
    type_face_to_face: "面对面",
    type_online: "在线",
    type_phone: "电话",
  },

  jrdTargetCandidateProfileForm: {
    title: "候选人画像",
    ideal_candidate_title: "理想候选人",
    ideal_candidate_hint:
      "曾成功担任过相同角色。入职第一天即可胜任，风险极低。",
    good_fit_title: "合适人选",
    good_fit_hint: "匹配度高，少量差距可在入职后补齐。",
    recommend_with_reservations_title: "有保留地推荐",
    recommend_with_reservations_hint:
      "具备相关经验、潜力高，属于「成长型」选择，需要较长适应期。",
    not_a_fit_title: "不合适",
    not_a_fit_hint:
      "不满足硬性条件，或存在在该岗位上可能失败的风险因素，即使背景看似相关。",
    featuresLabel: "特征描述",
    profileLabel: "典型画像",
    agree: "没有修改，提议很好。",
    back: "返回",
    submit: "提交修改",
    description: "描述",
    exampleProfiles: "示例画像：",
    profile: "画像",
    hint: "根据您分享的内容，我拟定了我们认为应该在搜索中定位的候选人画像。请随意添加、删除或修改项目——并拖放调整匹配度。",
  },

  jrdRealRequirementForm: {
    title: "真实需求",
    p0_title: "P0",
    p0_subTitle: "一票否决",
    p0_hint:
      "候选人必须在入职第一天就具备这些条件。只要有一条不满足，即视为不符合。",
    p0_note: "请尽量精简，避免过度缩小人才池。",
    p1_title: "P1",
    p1_subTitle: "强烈期望",
    p1_hint:
      "区分「能胜任」与「最理想」人选的关键能力。会显著提升候选人排序，但不作为通过/不通过的标准。",
    p2_title: "P2",
    p2_subTitle: "加分项",
    p2_hint:
      "可在工作中快速习得的技能，属于「加分」项，不应作为淘汰候选人的依据。",
    agree: "没有修改，提议很好。",
    back: "返回",
    submit: "提交修改",
    hint: "根据您分享的内容，我拟定了我们认为应该在搜索中定位的候选人画像。请随意添加、删除或修改项目——并拖放调整匹配度。",
  },
};
