...
describe('JobList 页面', () => {
  ...
  it('在搜索框输入关键词后只显示匹配的职位', async () => {
    ...
    const searchInput = screen.getByPlaceholderText('job_list.search_placeholder')
  ...
  it('选择创建人后只显示对应职位', async () => {
    ...
    const { PointerEventsCheckLevel } = await import('@testing-library/user-event')
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(mockJobList[0].name)).toBeInTheDocument()
      expect(screen.queryByText(mockJobList[1].name)).not.toBeInTheDocument()
    })
  })
  ...
  it('没有职位时显示空状态描述', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/description_line_1/),
      ).toBeInTheDocument()
    })
  })
  ...
  it('点击删除并确认后调用 destroy API，职位从列表消失', async () => {
    ...
    const okButton = within(confirmBtns as HTMLElement).getByRole('button', { name: /ok/i })
  ...
  it('点击取消删除后职位仍在列表中，destroy API 未被调用', async () => {
    ...
    const cancelButton = within(confirmBtns as HTMLElement).getByRole('button', { name: /cancel/i })
